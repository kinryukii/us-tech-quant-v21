...
def test_previous_physical_completion_references_violate_registry_firewall() -> None:
    registry = lifecycle._registry_module(Path(lifecycle.__file__).resolve().parent)
    physical_reference = (
...
def test_completion_uses_content_addressed_receipt_reference(tmp_path: Path) -> None:
    ...
    shutil.copy2(
        Path(lifecycle.__file__).with_name("research_registry.py"),
        repo / "research_registry.py",
    )
