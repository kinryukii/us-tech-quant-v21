#!/usr/bin/env python
"""Tests for V20.180 DATA_TRUST multiday observation run 3."""

from __future__ import annotations

import csv
import hashlib
import subprocess
from pathlib import Path


ROOT = Path(__file__).resolve().parents[2]
SCRIPT = ROOT / "scripts" / "v20" / "v20_180_data_trust_multiday_observation_run_3.py"
FACTORS = ROOT / "outputs" / "v20" / "factors"
CONSOLIDATION = ROOT / "outputs" / "v20" / "consolidation"
READ_CENTER = ROOT / "outputs" / "v20" / "read_center"

OUTPUTS = [
    FACTORS / "V20_180_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_3.csv",
    FACTORS / "V20_180_DATA_TRUST_RUN_3_CANDIDATE_DISCLOSURE_AUDIT.csv",
    FACTORS / "V20_180_DATA_TRUST_RUN_3_ZERO_WEIGHT_NO_MUTATION_AUDIT.csv",
    FACTORS / "V20_180_DATA_TRUST_RUN_3_OFFICIAL_USE_GUARD_AUDIT.csv",
    FACTORS / "V20_180_DATA_TRUST_RUN_3_VS_RUN_1_RUN_2_COMPARISON_AUDIT.csv",
    FACTORS / "V20_180_NEXT_STAGE_GATE.csv",
    READ_CENTER / "V20_180_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_3_REPORT.md",
]
PROTECTED = [
    CONSOLIDATION / "V20_83_AUTHORITATIVE_OFFICIAL_CURRENT_RANKING_TABLE.csv",
    CONSOLIDATION / "V20_98B_R1_FACTOR_WEIGHT_SOURCE_REGISTRY.csv",
    FACTORS / "V20_174_DATA_TRUST_DAILY_RUNNER_OUTPUT_SAMPLE.csv",
    FACTORS / "V20_177_NEXT_STAGE_GATE.csv",
    FACTORS / "V20_178_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_1.csv",
    FACTORS / "V20_178_DATA_TRUST_RUN_1_CANDIDATE_DISCLOSURE_AUDIT.csv",
    FACTORS / "V20_178_NEXT_STAGE_GATE.csv",
    FACTORS / "V20_179_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_2.csv",
    FACTORS / "V20_179_DATA_TRUST_RUN_2_CANDIDATE_DISCLOSURE_AUDIT.csv",
    FACTORS / "V20_179_NEXT_STAGE_GATE.csv",
]
PASS_STATUS = "PASS_V20_180_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_3_READY_FOR_V20_181_CLOSEOUT_SUMMARY"

SAFETY_FALSE_FIELDS = [
    "formal_activation_allowed", "promotion_ready", "official_recommendation_created",
    "official_ranking_mutated", "official_weight_change_created",
    "official_weight_registry_mutated", "weight_mutated", "real_book_action_created",
    "trade_action_created", "broker_execution_supported", "performance_claim_created",
    "shadow_weight_expansion_allowed",
]


def read_csv(path: Path) -> list[dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        return list(csv.DictReader(handle))


def digest(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def protected_hashes() -> dict[Path, str]:
    return {path: digest(path) for path in PROTECTED if path.exists()}


def assert_safety(rows: list[dict[str, str]]) -> None:
    for row in rows:
        for field in SAFETY_FALSE_FIELDS:
            if field in row:
                assert row[field] == "FALSE", f"{field} is not FALSE"
        if "data_trust_scoring_weight" in row:
            assert row["data_trust_scoring_weight"] == "0.0000000000"
        if "data_trust_role" in row:
            assert row["data_trust_role"] == "GATE_ONLY_AND_REPAIR_DIAGNOSTIC"


def test_data_trust_multiday_observation_run_3() -> None:
    before = protected_hashes()
    result = subprocess.run(["python", str(SCRIPT)], cwd=ROOT, text=True, capture_output=True, check=True)
    after = protected_hashes()
    assert before == after, "protected artifacts were mutated"
    stdout = result.stdout
    for expected in [
        PASS_STATUS,
        "OBSERVATION_RUN_SEQUENCE=3",
        "BASELINE_CANDIDATE_COUNT=40",
        "RUN_1_CANDIDATE_COUNT=40",
        "RUN_2_CANDIDATE_COUNT=40",
        "RUN_3_CANDIDATE_COUNT=40",
        "RUN_3_DATA_TRUST_DISCLOSURE_CANDIDATE_COUNT=40",
        "OFFICIAL_RANKING_SCORE_MUTATION_COUNT=0",
        "OFFICIAL_RANK_MUTATION_COUNT=0",
        "DATA_TRUST_SCORE_CONTRIBUTION_SUM=0.0000000000",
        "DATA_TRUST_NONZERO_WEIGHT_COUNT=0",
        "CANDIDATE_REMOVED_OR_REORDERED_COUNT=0",
        "RUN_TO_RUN_CANDIDATE_MISMATCH_COUNT=0",
        "RUN_TO_RUN_DATA_TRUST_CAUSED_RANKING_MUTATION_COUNT=0",
        "RUN_TO_RUN_DISCLOSURE_CHANGE_COUNT=0",
        "OFFICIAL_RECOMMENDATION_CREATED=FALSE",
        "REAL_BOOK_USE_ALLOWED=FALSE",
        "OFFICIAL_WEIGHT_MUTATION_ALLOWED=FALSE",
        "RUN_3_DISCLOSURE_PASS=TRUE",
        "RUN_3_NO_MUTATION_GUARD_PASS=TRUE",
        "RUN_3_ZERO_WEIGHT_GUARD_PASS=TRUE",
        "RUN_3_OFFICIAL_USE_GUARD_PASS=TRUE",
        "RUN_3_VS_RUN_1_RUN_2_COMPARISON_PASS=TRUE",
        "READY_FOR_V20_181_MULTIDAY_OBSERVATION_CLOSEOUT_SUMMARY=TRUE",
        "READY_FOR_OFFICIAL_USE=FALSE",
        "OFFICIAL_MUTATION_DETECTED=FALSE",
    ]:
        assert expected in stdout, expected
    for path in OUTPUTS:
        assert path.exists(), f"missing output {path}"

    run = read_csv(OUTPUTS[0])
    disclosure = read_csv(OUTPUTS[1])
    zero = read_csv(OUTPUTS[2])
    official = read_csv(OUTPUTS[3])
    comparison = read_csv(OUTPUTS[4])
    gate = read_csv(OUTPUTS[5])[0]
    assert len(run) == 1
    assert len(disclosure) == 40
    assert len(comparison) == 40
    assert all(row["run_3_disclosure_pass"] == "TRUE" for row in disclosure)
    assert all(row["data_trust_weight"] == "0.0000000000" for row in disclosure)
    assert all(row["guard_passed"] == "TRUE" for row in zero + official)
    assert all(row["run_3_vs_run_1_run_2_comparison_pass"] == "TRUE" for row in comparison)
    assert all(row["data_trust_caused_ranking_mutation_vs_run_1"] == "FALSE" for row in comparison)
    assert all(row["data_trust_caused_ranking_mutation_vs_run_2"] == "FALSE" for row in comparison)
    assert gate["final_status"] == PASS_STATUS
    assert gate["ready_for_v20_181_multiday_observation_closeout_summary"] == "TRUE"
    assert gate["ready_for_official_use"] == "FALSE"
    assert_safety([gate, *run, *disclosure[:3], *comparison[:3]])
    report = OUTPUTS[-1].read_text(encoding="utf-8")
    assert "Run 3 confirms" in report


if __name__ == "__main__":
    test_data_trust_multiday_observation_run_3()
    print("PASS test_v20_180_data_trust_multiday_observation_run_3")
