#!/usr/bin/env python
"""Tests for V20.158 reduced shadow weight ranking simulation."""
...
SCRIPT = ROOT / "scripts" / "v20" / "v20_158_reduced_shadow_weight_ranking_simulation.py"
...
OUT_REPORT = READ_CENTER / "V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION_REPORT.md"
OUTPUTS = [OUT_SIM, OUT_DELTA, OUT_GATE, OUT_SOURCE, OUT_SAFETY, OUT_COMPARISON, OUT_REPORT]
...
def upstream_hashes() -> dict[Path, str]:
    paths = [
        ...
        FACTORS / "V20_157_DELTA_REDUCTION_SOURCE_AUDIT.csv",
        FACTORS / "V20_157_DELTA_REDUCTION_SAFETY_AUDIT.csv",
        FACTORS / "V20_157_DELTA_REDUCTION_LIMITATION_AUDIT.csv",
...
def load_module():
    spec = importlib.util.spec_from_file_location("v20_158_reduced_shadow_weight_ranking_simulation_case", SCRIPT)
...
def patch_module_to_temp(module, temp: Path) -> None:
    ...
    module.IN_PROPOSAL = module.FACTORS / "V20_157_REDUCED_SHADOW_DYNAMIC_WEIGHT_PROPOSAL.csv"
    module.IN_GATE = module.FACTORS / "V20_157_DELTA_REDUCTION_GATE.csv"
    module.IN_SOURCE = module.FACTORS / "V20_157_DELTA_REDUCTION_SOURCE_AUDIT.csv"
    module.IN_SAFETY = module.FACTORS / "V20_157_DELTA_REDUCTION_SAFETY_AUDIT.csv"
    module.IN_LIMITATION = module.FACTORS / "V20_157_DELTA_REDUCTION_LIMITATION_AUDIT.csv"
    ...
    module.V155_SIM = module.FACTORS / "V20_155_LIMITED_SHADOW_WEIGHT_RANKING_SIMULATION.csv"
    module.V155_DELTA = module.FACTORS / "V20_155_SHADOW_VS_BASELINE_RANK_DELTA.csv"
    module.OUT_SIM = module.FACTORS / "V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION.csv"
    module.OUT_DELTA = module.FACTORS / "V20_158_REDUCED_SHADOW_VS_BASELINE_RANK_DELTA.csv"
    module.OUT_GATE = module.FACTORS / "V20_158_REDUCED_SHADOW_RANKING_GATE.csv"
    module.OUT_SOURCE = module.FACTORS / "V20_158_REDUCED_SHADOW_RANKING_SOURCE_AUDIT.csv"
    module.OUT_SAFETY = module.FACTORS / "V20_158_REDUCED_SHADOW_RANKING_SAFETY_AUDIT.csv"
    module.OUT_COMPARISON = module.FACTORS / "V20_158_REDUCED_VS_ORIGINAL_SHADOW_IMPACT_COMPARISON.csv"
    module.REPORT = module.READ_CENTER / "V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION_REPORT.md"
...
def copy_inputs(temp: Path) -> None:
    ...
    for filename in [
        "V20_157_REDUCED_SHADOW_DYNAMIC_WEIGHT_PROPOSAL.csv",
        "V20_157_DELTA_REDUCTION_GATE.csv",
        "V20_157_DELTA_REDUCTION_SOURCE_AUDIT.csv",
        "V20_157_DELTA_REDUCTION_SAFETY_AUDIT.csv",
        "V20_157_DELTA_REDUCTION_LIMITATION_AUDIT.csv",
...
def assert_safety(rows: list[dict[str, str]]) -> None:
    for row in rows:
        ...
        if "reduced_shadow_ranking_simulation_created" in row:
            assert row["reduced_shadow_ranking_simulation_created"] == "TRUE"
        if "reduced_shadow_ranking_simulation_scope" in row:
            assert row["reduced_shadow_ranking_simulation_scope"] == "RESEARCH_ONLY_LIMITED_CONSERVATIVE"
...
def write_minimal_inputs(module, status_ok: bool = True) -> None:
    write_csv(module.IN_PROPOSAL, [{
        ...
        "reduced_proposed_delta": "-0.0015000000",
        "reduction_multiplier": "0.2500000000",
        "confidence_level": "LIMITED",
        "evidence_quality": "HIGH",
        "usable_for_reduced_shadow_simulation": "TRUE",
        "usable_for_official_weight_change": "FALSE",
    }])
    write_csv(module.IN_GATE, [{
        "final_status": "PASS_V20_157_SHADOW_WEIGHT_DELTA_REDUCTION_READY_FOR_V20_158" if status_ok else "WARN_V20_157_NO_REDUCIBLE_SHADOW_DELTAS_FOUND",
        "v20_158_reduced_shadow_simulation_allowed": "TRUE" if status_ok else "FALSE",
    }])
    ...
    write_csv(module.V155_SIM, [
        {"ticker": "AAA", "shadow_rank": "2", "rank_delta": "-1"},
        {"ticker": "BBB", "shadow_rank": "1", "rank_delta": "1"},
    ])
    write_csv(module.V155_DELTA, [{
        "baseline_candidate_count": "2",
        "entered_top20_count": "0",
        "exited_top20_count": "0",
        "max_absolute_rank_delta": "1",
        "average_absolute_rank_delta": "1",
...
def test_blocked_missing_inputs_case() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert gate["final_status"] == "BLOCKED_V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION"


def test_blocked_wrong_v157_status_case() -> None:
    module = load_module()
    with tempfile.TemporaryDirectory() as temp_dir:
        temp = Path(temp_dir)
        patch_module_to_temp(module, temp)
...
def test_temp_reduced_shadow_simulation_case() -> None:
    module = load_module()
    with tempfile.TemporaryDirectory() as temp_dir:
        temp = Path(temp_dir)
        patch_module_to_temp(module, temp)
        ...
        comparison = read_csv(module.OUT_COMPARISON)
        assert gate["final_status"] in {
            "PASS_V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION_READY_FOR_V20_159",
            "PARTIAL_PASS_V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION_WITH_REMAINING_INSTABILITY_READY_FOR_V20_159",
            "WARN_V20_158_REDUCED_SHADOW_RANKING_STILL_TOO_UNSTABLE",
        }
        assert len(sim) == 2
        assert sim[0]["official_ranking_mutated"] == "FALSE"
        assert comparison[0]["impact_reduction_success"] in {"TRUE", "FALSE"}


def test_reduced_shadow_weight_ranking_simulation() -> None:
    before = upstream_hashes()
    result = subprocess.run(["python", str(SCRIPT)], cwd=ROOT, text=True, capture_output=True, check=True)
    after = upstream_hashes()
    assert before == after, "upstream V20.155/V20.157 outputs were mutated"
    ...
    for expected in [
        ...
        "V20_157_ALLOWED_FOR_V20_158=TRUE",
        ...
        "REDUCED_SHADOW_RANKING_SIMULATION_SCOPE=RESEARCH_ONLY_LIMITED_CONSERVATIVE",
    ...
    comparison = read_csv(OUT_COMPARISON)
    ...
    assert comparison, "comparison output must not be empty"
    assert SIM_COLUMNS.issubset(sim[0].keys()), SIM_COLUMNS - set(sim[0].keys())
    assert COMPARISON_COLUMNS.issubset(comparison[0].keys()), COMPARISON_COLUMNS - set(comparison[0].keys())
    ...
    assert comparison[0]["remaining_rank_impact_severity"] in {"LOW", "ELEVATED", "HIGH", "EXTREME"}
    assert comparison[0]["recommended_next_action"] in {
        "APPROVE_CONTINUED_REDUCED_SHADOW_RESEARCH_ONLY",
        "REQUEST_OPERATOR_REVIEW_BEFORE_ANY_SHADOW_EXPANSION",
        "REQUEST_ADDITIONAL_DELTA_REDUCTION_OR_REJECT_SHADOW_DYNAMIC_WEIGHT_PATH_FOR_NOW",
    }
    for rows in [sim, delta, gate, source, safety, comparison]:
        assert_safety(rows)


if __name__ == "__main__":
    test_blocked_missing_inputs_case()
    test_blocked_wrong_v157_status_case()
    test_temp_reduced_shadow_simulation_case()
    test_reduced_shadow_weight_ranking_simulation()
    print("PASS_V20_158_REDUCED_SHADOW_WEIGHT_RANKING_SIMULATION_TESTS")
