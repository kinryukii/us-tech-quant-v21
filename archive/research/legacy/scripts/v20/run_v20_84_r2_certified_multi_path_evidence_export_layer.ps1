...
$StageScript = Join-Path $ScriptDir "v20_84_r2_certified_multi_path_evidence_export_layer.py"

Write-Host "STAGE_NAME=V20.84_R2_CERTIFIED_MULTI_PATH_EVIDENCE_EXPORT_LAYER"
...
try {
    ...
    if (-not $DetectedStatus) {
        throw "V20.84-R2 did not report an allowed final status"
    }
    if ($ExitCode -ne 0) {
        throw "V20.84-R2 reported non-zero exit code $ExitCode"
    }
