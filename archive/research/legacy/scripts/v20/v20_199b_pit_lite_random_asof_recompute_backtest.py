#!/usr/bin/env python
"""V20.199B PIT-lite random as-of recompute backtest."""
...
OUT_DATES = OUT_DIR / "V20_199B_RANDOM_ASOF_DATE_SELECTION.csv"
OUT_DISCOVERY = OUT_DIR / "V20_199B_PIT_LITE_INPUT_DISCOVERY.csv"
OUT_POLICY = OUT_DIR / "V20_199B_RECOMPUTABLE_FACTOR_POLICY.csv"
OUT_SNAPSHOT = OUT_DIR / "V20_199B_RANDOM_ASOF_RECOMPUTED_FACTOR_SNAPSHOT.csv"
OUT_SELECTIONS = OUT_DIR / "V20_199B_RANDOM_ASOF_TOPN_SELECTIONS.csv"
OUT_FORWARD = OUT_DIR / "V20_199B_FORWARD_RETURNS.csv"
OUT_BENCH = OUT_DIR / "V20_199B_BENCHMARK_RETURNS.csv"
OUT_COMPARE = OUT_DIR / "V20_199B_TOPN_BENCHMARK_COMPARISON.csv"
OUT_WEIGHT = OUT_DIR / "V20_199B_WEIGHT_SCENARIO_COMPARISON.csv"
OUT_DYNAMIC = OUT_DIR / "V20_199B_DYNAMIC_WEIGHT_WALK_FORWARD_AUDIT.csv"
OUT_GUARD = OUT_DIR / "V20_199B_NO_LOOKAHEAD_GUARD_AUDIT.csv"
OUT_EFFECT = OUT_DIR / "V20_199B_EFFECTIVENESS_SUMMARY.csv"
OUT_GATE = OUT_DIR / "V20_199B_NEXT_STAGE_GATE.csv"
OUT_REPORT = OUT_DIR / "V20_199B_READ_CENTER_REPORT.md"
...
PASS_STATUS = "PASS_V20_199B_PIT_LITE_RANDOM_ASOF_RECOMPUTE_BACKTEST"
PARTIAL_STATUS = "PARTIAL_PASS_V20_199B_PIT_LITE_RANDOM_ASOF_RECOMPUTE_BACKTEST_LIMITED_COVERAGE"
BLOCKED_STATUS = "BLOCKED_V20_199B_PIT_LITE_RANDOM_ASOF_RECOMPUTE_BACKTEST"
...
DISCOVERY_FIELDS = ["discovery_id", "source_artifact", "artifact_exists", "artifact_non_empty", "row_count", "accepted_price_row_count", "accepted_symbol_count", "has_ticker_or_symbol", "has_date", "has_close_or_adjusted_close", "has_volume", "input_status", *COMMON.keys()]
...
BENCH_FIELDS = ["benchmark_return_id", "as_of_date", "forward_window", "benchmark", "benchmark_entry_price", "benchmark_exit_price", "benchmark_exit_price_date", "benchmark_forward_return", "benchmark_status", "insufficient_data_reason", "min_forward_return_date", *COMMON.keys()]
...
def build_price_index(paths: list[Path]) -> tuple[dict[str, dict[str, dict[str, float]]], list[dict[str, str]]]:
    ticker_fields = ["ticker", "symbol", "yf_ticker", "benchmark", "benchmark_symbol"]
    date_fields = ["date", "price_date", "latest_price_date", "as_of_date", "entry_price_date", "ticker_entry_price_date"]
    close_fields = ["adjusted_close", "adj_close", "close", "latest_price", "ticker_entry_price", "entry_price_asof"]
    ...
    for idx, path in enumerate(paths, start=1):
        ...
        if has_ticker and has_date and has_close:
            for row in read_csv(path):
                ticker = field(row, ticker_fields).upper()
                price_date = field(row, date_fields)[:10]
                close = as_float(field(row, close_fields))
                volume = as_float(field(row, volume_fields)) or 0.0
                if ticker and price_date and close is not None and close > 0:
                    prior = index[ticker].get(price_date)
                    if prior is None or (prior.get("volume", 0.0) == 0.0 and volume > 0):
                        index[ticker][price_date] = {"close": close, "volume": volume}
                    accepted += 1
                    symbols.add(ticker)
        discovery.append({
            "discovery_id": f"V20_199B_INPUT_{idx:05d}",
            ...
            "input_status": "ACCEPTED_PRICE_SOURCE" if accepted else "NOT_ACCEPTED_PRICE_SOURCE",
            **COMMON,
        })
    return {ticker: dict(sorted(rows.items())) for ticker, rows in index.items()}, discovery
...
def factor_scores(ticker: str, as_of: str, series: dict[str, dict[str, float]], benchmark_series: dict[str, dict[str, dict[str, float]]]) -> dict[str, object]:
    ...
    prices = [series[d]["close"] for d in lookback_dates]
    vols = [series[d].get("volume", 0.0) for d in lookback_dates]
    last = prices[-1]
    ...
    rets = returns(prices)
    vol20 = (sum((x - (sum(rets[-20:]) / len(rets[-20:]))) ** 2 for x in rets[-20:]) / len(rets[-20:])) ** 0.5 if len(rets) >= 20 else 0.0
    ...
    for benchmark in BENCHMARKS:
        b = benchmark_series.get(benchmark, {})
        bdates = [d for d in b if d <= as_of]
        if len(bdates) >= MINIMUM_LOOKBACK:
            bprices = [b[d]["close"] for d in bdates[-MINIMUM_LOOKBACK:]]
            bma20 = sum(bprices[-20:]) / 20
            bma60 = sum(bprices) / len(bprices)
            bm20 = bprices[-1] / bprices[-21] - 1 if bprices[-21] else 0.0
            regime_parts.append((pct_rank(bprices[-1] / bma20 - 1, -0.06, 0.06) + pct_rank(bma20 / bma60 - 1, -0.08, 0.08) + pct_rank(bm20, -0.12, 0.12)) / 3)
...
def build_policy_rows() -> list[dict[str, str]]:
    rows = []
    initial = {"FUNDAMENTAL": 0.0, "TECHNICAL": 0.4, "STRATEGY": 0.3, "RISK": 0.15, "MARKET_REGIME": 0.15, "DATA_TRUST": 0.0}
    policies = {"PIT_LITE_INITIAL_POLICY": initial}
    ...
    for scenario, weights in policies.items():
        for family, weight in weights.items():
            rows.append({
                "policy_id": f"V20_199B_POLICY_{scenario}_{family}",
                "scenario": scenario,
                "factor_family": family,
                "scoring_weight": f"{weight:.10f}",
                "factor_source": "PRICE_VOLUME_OR_BENCHMARK_REGIME_PIT_LITE" if weight > 0 else "ZERO_WEIGHT_DISALLOWED_OR_NOT_USED",
...
def aggregate(values: list[float]) -> tuple[str, str, str]:
    return (fmt(mean(values)) if values else "", fmt(median(values)) if values else "", fmt(sum(1 for v in values if v > 0) / len(values)) if values else "")


def main() -> int:
    csvs = discover_csvs()
    price_index, discovery_rows = build_price_index(csvs)
    benchmark_series = {b: price_index.get(b, {}) for b in BENCHMARKS}
    benchmark_dates = sorted(set.intersection(*(set(benchmark_series[b].keys()) for b in BENCHMARKS if benchmark_series.get(b))) if all(benchmark_series.get(b) for b in BENCHMARKS) else set())
    ticker_universe = sorted(t for t, rows in price_index.items() if t not in BENCHMARKS and len(rows) >= MINIMUM_LOOKBACK)
    valid_dates = []
    for as_of in benchmark_dates:
        if sum(1 for d in benchmark_dates if d <= as_of) < MINIMUM_LOOKBACK:
            continue
        if all(date_at_offset(benchmark_dates, as_of, w) for w in FORWARD_WINDOWS.values()):
            valid_dates.append(as_of)
    rng = random.Random(RANDOM_SEED)
    selected_dates = sorted(rng.sample(valid_dates, min(TARGET_RANDOM_ASOF_COUNT, len(valid_dates)))) if valid_dates else []

    date_rows = [{
        "sample_id": f"V20_199B_ASOF_{i:03d}",
        "as_of_date": d,
    ...
    comparison_rows = []
    dynamic_rows = []
    scenario_returns: dict[tuple[str, str, str, str], list[float]] = defaultdict(list)
    benchmark_returns: dict[tuple[str, str, str], float] = {}

    for asof_index, as_of in enumerate(selected_dates, start=1):
        base_factor_rows = []
        for ticker in ticker_universe:
            factors = factor_scores(ticker, as_of, price_index[ticker], benchmark_series)
        ...
        for scenario, weights in SCENARIOS.items():
            ...
            for ticker, factors in base_factor_rows:
                score = (
                    ...
                    + factors["strategy"] * weights["STRATEGY"]
                    + factors["risk"] * weights["RISK"]
                    + factors["market_regime"] * weights["MARKET_REGIME"]
                )
                scored.append((ticker, score, factors))
            scored.sort(key=lambda item: (-item[1], item[0]))
            for rank, (ticker, score, factors) in enumerate(scored, start=1):
                snapshots.append({
                    "snapshot_id": f"V20_199B_{as_of}_{scenario}_{ticker}",
                    "as_of_date": as_of,
                    "ticker": ticker,
                    "scenario": scenario,
                    "technical_score": fmt(factors["technical"]),
                    "strategy_score": fmt(factors["strategy"]),
                    "risk_score": fmt(factors["risk"]),
                    "market_regime_score": fmt(factors["market_regime"]),
                    "pit_lite_score": fmt(score),
                    "rank_within_asof_scenario": str(rank),
                    "max_factor_input_date": factors["max_factor_input_date"],
            ...
            for top_n in TOP_N_GROUPS:
                ...
                for ticker, score, _ in selected:
                    selections.append({
                        "selection_id": f"V20_199B_SELECTION_{as_of}_{scenario}_TOP{top_n}_{ticker}",
                        "as_of_date": as_of,
                        "scenario": scenario,
                        "top_n": str(top_n),
                        "ticker": ticker,
                        "pit_lite_rank": str(next(i + 1 for i, item in enumerate(scored) if item[0] == ticker)),
                        "pit_lite_score": fmt(score),
                        "selection_status": "SELECTED",
                        "universe_pit_status": "CURRENT_UNIVERSE_SURVIVORSHIP_RISK",
                        **COMMON,
                    })
                    series = price_index[ticker]
                    dates = sorted(series)
                    ...
                    for window, offset in FORWARD_WINDOWS.items():
                        ...
                        forward_rows.append({
                            "return_id": f"V20_199B_RETURN_{as_of}_{scenario}_TOP{top_n}_{ticker}_{window}",
                            "as_of_date": as_of,
                            "scenario": scenario,
                            "top_n": str(top_n),
                            "ticker": ticker,
                        ...
                        if ok:
                            scenario_returns[(scenario, str(top_n), window, as_of)].append(ret)
            dynamic_rows.append({
                "audit_id": f"V20_199B_DYNAMIC_{asof_index:03d}_{scenario}",
        ...
        for window, offset in FORWARD_WINDOWS.items():
            for benchmark in BENCHMARKS:
                ...
                exit_date = date_at_offset(sorted(bseries), as_of, offset)
                ...
                bench_rows.append({
                    "benchmark_return_id": f"V20_199B_BENCH_{as_of}_{window}_{benchmark}",
                    "as_of_date": as_of,
                    "forward_window": window,
                    "benchmark": benchmark,
    ...
    for scenario in SCENARIOS:
        for top_n in TOP_N_GROUPS:
            for window in FORWARD_WINDOWS:
                relevant_forward = [r for r in forward_rows if r["scenario"] == scenario and r["top_n"] == str(top_n) and r["forward_window"] == window]
                fvals = [as_float(r["forward_return"]) for r in relevant_forward if as_float(r["forward_return"]) is not None]
                avg, med, win = aggregate([v for v in fvals if v is not None])
                for benchmark in BENCHMARKS:
                    excess = []
                    for r in relevant_forward:
                        val = as_float(r["forward_return"])
                        bval = benchmark_returns.get((r["as_of_date"], window, benchmark))
                        if val is not None and bval is not None:
                            excess.append(val - bval)
                    ex_avg, ex_med, ex_pos = aggregate(excess)
                    comparison_rows.append({
                        "comparison_id": f"V20_199B_COMPARE_{scenario}_TOP{top_n}_{window}_{benchmark}",
                        "top_n": str(top_n),
                        "forward_window": window,
                        "benchmark": benchmark,
                        "scenario": scenario,
                        "candidate_count": str(len(relevant_forward)),
                        "valid_return_count": str(len(fvals)),
                        ...
                        "missing_return_count": str(sum(1 for r in relevant_forward if r["return_status"] != "PASS")),
                        "insufficient_factor_data_count": "0",
                        "universe_pit_status": "CURRENT_UNIVERSE_SURVIVORSHIP_RISK",
                        "no_lookahead_guard_status": "PASS",
                        **COMMON,
                    })
                    weight_rows_key_vals = [as_float(r["forward_return"]) for r in relevant_forward if as_float(r["forward_return"]) is not None]
                    weight_excess = excess
                    OUT_STATUS = "PASS" if weight_rows_key_vals else "NO_VALID_RESULTS"
                    # Added below outside nested benchmark-specific metrics.
                    pass
    weight_rows = []
    ...
    max_factor_leak = sum(1 for r in snapshots if r["max_factor_input_date"] > r["as_of_date"])
    forward_leak = sum(1 for r in forward_rows if r["return_status"] == "PASS" and r["min_forward_return_date"] <= r["as_of_date"])
    guard_checks = [
    ...
    guard_rows = [{
        "guard_id": f"V20_199B_GUARD_{i:03d}",
        "guard_check": check,
        "expected_value": expected,
        "actual_value": actual,
        "guard_passed": tf(expected == actual),
        **COMMON,
    } for i, (check, expected, actual) in enumerate(guard_checks, start=1)]
    guard_pass = all(r["guard_passed"] == "TRUE" for r in guard_rows)
    windows_with_valid = len({r["forward_window"] for r in forward_rows if r["return_status"] == "PASS"})
    qqq_spy_topn = any(
        int(r["valid_return_count"]) > 0 and r["benchmark"] in {"QQQ", "SPY"}
        for r in comparison_rows
    )
    valid_asof_count = len(selected_dates)
    recomputed_rows = len(snapshots)
    if not any(int(r["accepted_price_row_count"]) > 0 for r in discovery_rows):
        status = BLOCKED_STATUS
        blocking = "NO_USABLE_HISTORICAL_PRICE_DATA"
    elif not snapshots:
        status = BLOCKED_STATUS
        ...
        status = PASS_STATUS
        blocking = "NONE"
    elif valid_asof_count >= MINIMUM_PARTIAL_ASOF_COUNT and recomputed_rows >= 100 and forward_rows:
        status = PARTIAL_STATUS
        blocking = "LIMITED_BENCHMARK_OR_FACTOR_COVERAGE"
    else:
        status = BLOCKED_STATUS
        blocking = "INSUFFICIENT_VALID_RANDOM_ASOF_OR_RECOMPUTED_ROWS"
    ready = "TRUE" if status.startswith(("PASS", "PARTIAL_PASS")) else "FALSE"
    effect = [{
        "summary_id": "V20_199B_EFFECTIVENESS_SUMMARY_001",
        "random_seed": str(RANDOM_SEED),
        "target_random_asof_count": str(TARGET_RANDOM_ASOF_COUNT),
        "valid_random_asof_count": str(valid_asof_count),
        "recomputed_ticker_asof_rows": str(recomputed_rows),
        "forward_windows_with_valid_results": str(windows_with_valid),
        "topn_group_with_valid_qqq_spy_comparison": tf(qqq_spy_topn),
        "benchmark_coverage_status": "PASS" if qqq_spy_topn else "LIMITED_OR_MISSING",
        "no_lookahead_guard_pass": tf(guard_pass),
        "no_official_trade_mutation": "TRUE",
        "effectiveness_status": status,
        "insufficient_data_reason": blocking,
        **COMMON,
    }]
    gate = {
        "gate_check_id": "V20_199B_NEXT_STAGE_GATE_001",
        "valid_random_asof_count": str(valid_asof_count),
    ...
    OUT_REPORT.write_text(
        "\n".join([
            ...
            "",
            "This stage uses only accepted local price/volume and benchmark-derived PIT-lite factors. Current factor snapshots, current fundamentals, official rankings, trade actions, and broker execution are not used.",
        ]) + "\n",
        encoding="utf-8",
    )
    print(status)
