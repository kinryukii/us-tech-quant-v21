#!/usr/bin/env python
"""Tests for V20.171 DATA_TRUST full gate-only ranking simulation."""

from __future__ import annotations

import csv
import hashlib
import subprocess
from pathlib import Path


ROOT = Path(__file__).resolve().parents[2]
SCRIPT = ROOT / "scripts" / "v20" / "v20_171_data_trust_full_gate_only_ranking_simulation.py"
FACTORS = ROOT / "outputs" / "v20" / "factors"
CONSOLIDATION = ROOT / "outputs" / "v20" / "consolidation"
READ_CENTER = ROOT / "outputs" / "v20" / "read_center"

OUTPUTS = [
    FACTORS / "V20_171_DATA_TRUST_GATE_ONLY_RANKING_SIMULATION.csv",
    FACTORS / "V20_171_DATA_TRUST_GATE_ACTION_AUDIT.csv",
    FACTORS / "V20_171_RANKING_DELTA_VS_BASELINE_AUDIT.csv",
    FACTORS / "V20_171_DATA_TRUST_SCORE_CONTRIBUTION_AUDIT.csv",
    FACTORS / "V20_171_OFFICIAL_RANKING_MUTATION_GUARD_AUDIT.csv",
    FACTORS / "V20_171_NEXT_STAGE_GATE.csv",
    READ_CENTER / "V20_171_DATA_TRUST_FULL_GATE_ONLY_RANKING_SIMULATION_REPORT.md",
]
PROTECTED = [
    CONSOLIDATION / "V20_83_AUTHORITATIVE_OFFICIAL_CURRENT_RANKING_TABLE.csv",
    CONSOLIDATION / "V20_98B_R1_FACTOR_WEIGHT_SOURCE_REGISTRY.csv",
    FACTORS / "V20_170_R3_R2_DATA_TRUST_DIRECT_STATUS_RETEST_CANDIDATES.csv",
    FACTORS / "V20_170_R3_R2_NEXT_STAGE_GATE.csv",
]
PASS_STATUS = "PASS_V20_171_DATA_TRUST_FULL_GATE_ONLY_RANKING_SIMULATION_READY_FOR_V20_172"

SAFETY_FALSE_FIELDS = [
    "formal_activation_allowed", "promotion_ready", "official_recommendation_created",
    "official_ranking_mutated", "official_weight_change_created",
    "official_weight_registry_mutated", "weight_mutated", "real_book_action_created",
    "trade_action_created", "broker_execution_supported", "performance_claim_created",
    "shadow_weight_expansion_allowed",
]


def read_csv(path: Path) -> list[dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        return list(csv.DictReader(handle))


def digest(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def protected_hashes() -> dict[Path, str]:
    return {path: digest(path) for path in PROTECTED if path.exists()}


def assert_safety(rows: list[dict[str, str]]) -> None:
    for row in rows:
        for field in SAFETY_FALSE_FIELDS:
            if field in row:
                assert row[field] == "FALSE", f"{field} is not FALSE"
        if "data_trust_scoring_weight" in row:
            assert row["data_trust_scoring_weight"] == "0.0000000000"
        if "data_trust_role" in row:
            assert row["data_trust_role"] == "GATE_ONLY_AND_REPAIR_DIAGNOSTIC"


def test_data_trust_full_gate_only_ranking_simulation() -> None:
    before = protected_hashes()
    result = subprocess.run(["python", str(SCRIPT)], cwd=ROOT, text=True, capture_output=True, check=True)
    after = protected_hashes()
    assert before == after, "protected artifacts were mutated"
    stdout = result.stdout
    for expected in [
        PASS_STATUS,
        "BASELINE_CANDIDATE_COUNT=40",
        "SIMULATION_CANDIDATE_COUNT=40",
        "DATA_TRUST_PASS_CANDIDATE_COUNT=40",
        "DATA_TRUST_WARN_CANDIDATE_COUNT=0",
        "DATA_TRUST_FAIL_CANDIDATE_COUNT=0",
        "DATA_TRUST_UNKNOWN_CANDIDATE_COUNT=0",
        "OFFICIAL_RANKING_SCORE_MUTATION_COUNT=0",
        "OFFICIAL_RANK_MUTATION_COUNT=0",
        "DATA_TRUST_SCORE_CONTRIBUTION_SUM=0.0000000000",
        "DATA_TRUST_NONZERO_WEIGHT_COUNT=0",
        "OFFICIAL_RANKING_MUTATION_GUARD_PASS=TRUE",
        "DATA_TRUST_ZERO_WEIGHT_GUARD_PASS=TRUE",
        "DATA_TRUST_GATE_COVERAGE_PASS=TRUE",
        "READY_FOR_V20_172_IMPACT_STABILITY_DISCLOSURE_VALIDATION=TRUE",
        "READY_FOR_OFFICIAL_USE=FALSE",
        "REAL_BOOK_USE_ALLOWED=FALSE",
        "OFFICIAL_RECOMMENDATION_CREATED=FALSE",
        "OFFICIAL_MUTATION_DETECTED=FALSE",
    ]:
        assert expected in stdout, expected
    for path in OUTPUTS:
        assert path.exists(), f"missing output {path}"

    sim = read_csv(OUTPUTS[0])
    action = read_csv(OUTPUTS[1])
    delta = read_csv(OUTPUTS[2])
    contrib = read_csv(OUTPUTS[3])
    guards = read_csv(OUTPUTS[4])
    gate = read_csv(OUTPUTS[5])[0]
    assert len(sim) == 40
    assert len(action) == 4
    assert len(delta) == 40
    assert len(contrib) == 40
    assert all(row["data_trust_direct_status"] == "PASS" for row in sim)
    assert all(row["official_score_delta"] == "0" for row in sim)
    assert all(row["data_trust_official_weight"] == "0.0000000000" for row in sim)
    assert all(row["data_trust_score_contribution"] == "0.0000000000" for row in sim)
    assert all(row["candidate_removed_from_official_ranking"] == "FALSE" for row in sim)
    assert all(row["rank_delta"] == "0" for row in delta)
    assert all(row["zero_weight_guard_pass"] == "TRUE" for row in contrib)
    assert all(row["guard_passed"] == "TRUE" for row in guards)
    assert gate["final_status"] == PASS_STATUS
    assert gate["ready_for_v20_172_impact_stability_disclosure_validation"] == "TRUE"
    assert gate["ready_for_official_use"] == "FALSE"
    assert_safety([gate, *sim[:3], *delta[:3], *contrib[:3]])
    report = OUTPUTS[-1].read_text(encoding="utf-8")
    assert "zero official weight" in report


if __name__ == "__main__":
    test_data_trust_full_gate_only_ranking_simulation()
    print("PASS test_v20_171_data_trust_full_gate_only_ranking_simulation")
