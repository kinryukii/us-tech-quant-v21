...
2. 使用既有 [研究注册表](scripts/maintenance/research_registry.py) 查询研究身份及接受状态；注册表位置由 [config/research_registry.json](config/research_registry.json) 指定。
3. 对照已有机制、信息来源、目标期限及评价设计，检查已关闭、暂停、失败的工作，以及[已删除源码索引](docs/research/retired_sources.json)。删除文件或改名不会重置研究身份。
...
& D:\us-tech-quant-envs\us-tech-quant-main\Scripts\python.exe -B -m scripts.maintenance.research_registry current
& D:\us-tech-quant-envs\us-tech-quant-main\Scripts\python.exe -B -m scripts.maintenance.research_registry query --alias "已有名称或别名"
& D:\us-tech-quant-envs\us-tech-quant-main\Scripts\python.exe -B -m scripts.maintenance.research_registry preflight-proposal --proposal <提案JSON路径>
```
...
**身份状态与旧研究结论分开保留。** 旧表中的关闭原因、负面结论不会被新身份状态覆盖；不一致时标记待复核，不自动重启研究。查询同时覆盖当前源码和已删除源码的恢复索引；未命中仍需通过提案检查，避免换名重复已经完成的工作。
...
| `docs/` | 使用说明、项目地图与研究契约 |
| `docs/research/retired_sources.json` | 已删除源码的旧路径、哈希和 Git 恢复位置；参与查重 |

根目录只保留项目入口文档与必要配置。治理实现和测试已归入相应目录；确认不被当前程序依赖的历史源码已删除，不再另存一整套归档代码。确需恢复时，按索引定位[整理前的 Git 提交](https://github.com/kinryukii/us-tech-quant-v21/tree/3d0783a8c864552273394358268292f6d389e99b)。仍被运行链依赖的旧实现继续保留，具体位置见[仓库布局](docs/governance/REPOSITORY_LAYOUT.md)。
