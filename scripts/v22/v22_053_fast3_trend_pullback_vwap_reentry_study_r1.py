...
VERSION = "V22.053_FAST3_TREND_PULLBACK_VWAP_REENTRY_STUDY_R1"
...
def validate_v22_052(summary: Mapping[str, Any]) -> None:
    expected = {
        "final_status": "PASS",
    ...
    if failed:
        raise StudyError(
            "V22.052 failed-architecture lineage validation failed: "
            + ", ".join(failed)
        )
...
def exact_elapsed_return(series: pd.Series, minutes: int) -> pd.Series:
    prior = series.reindex(
        series.index - pd.Timedelta(minutes=minutes)
...
def add_daily_indicators(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["vwap"] = (
        cumulative_value.divide(
            cumulative_volume.where(cumulative_volume > 0)
        ).fillna(fallback)
    )

    result["ret_15m"] = np.nan
    result["ret_30m"] = np.nan
    result["vwap_slope_15m"] = np.nan

    for _, day in result.groupby("trade_date", sort=False):
        result.loc[day.index, "ret_15m"] = exact_elapsed_return(
            day["close"],
            15,
        )
        result.loc[day.index, "ret_30m"] = exact_elapsed_return(
            day["close"],
            30,
        )
        result.loc[day.index, "vwap_slope_15m"] = (
            exact_elapsed_return(day["vwap"], 15)
        )

    return result


def _add_pullback_columns(
    frame: pd.DataFrame,
    prefix: str,
) -> pd.DataFrame:
    result = frame.copy()
    touch = (
        (result[f"{prefix}_low"] <= result[f"{prefix}_vwap"])
        & (result[f"{prefix}_high"] >= result[f"{prefix}_vwap"])
    )
    close_above = (
        result[f"{prefix}_close"] >= result[f"{prefix}_vwap"]
    )

    result[f"{prefix}_previous_high"] = np.nan
    result[f"{prefix}_touch_prior_5m"] = False
    result[f"{prefix}_all_closes_above_vwap_prior_5m"] = False

    for _, day in result.groupby("trade_date", sort=False):
        indices = day.index
        day_touch = touch.loc[indices].shift(1)
        day_close_above = close_above.loc[indices].shift(1)

        result.loc[indices, f"{prefix}_previous_high"] = (
            day[f"{prefix}_high"].shift(1)
        )
        result.loc[indices, f"{prefix}_touch_prior_5m"] = (
            day_touch.rolling(5, min_periods=5).max().fillna(False)
            .astype(bool)
        )
        result.loc[
            indices,
            f"{prefix}_all_closes_above_vwap_prior_5m",
        ] = (
            day_close_above.rolling(5, min_periods=5).min()
            .fillna(False)
            .astype(bool)
        )
    return result
...
def build_aligned_signal_frame(
    qqq: pd.DataFrame,
    soxx: pd.DataFrame,
) -> pd.DataFrame:
    q = add_daily_indicators(qqq)[
        ...
        columns={
            ...
            "ret_15m": "qqq_ret_15m",
            "ret_30m": "qqq_ret_30m",
            "vwap_slope_15m": "qqq_vwap_slope_15m",
        }
    )
    s = add_daily_indicators(soxx)[
        ...
        columns={
            ...
            "ret_30m": "soxx_ret_30m",
            "vwap_slope_15m": "soxx_vwap_slope_15m",
        }
    )

    merged = q.join(s, how="inner")
    merged = merged.loc[
        merged["trade_date"] == merged["soxx_trade_date"]
    ].copy()
    merged = _add_pullback_columns(merged, "qqq")
    merged = _add_pullback_columns(merged, "soxx")
    ...
    return merged.loc[
        (minute >= SIGNAL_START_MINUTE)
        & (minute <= SIGNAL_END_MINUTE)
    ].sort_index()


def pullback_reentry_condition_and_branch(
    row: pd.Series,
) -> tuple[bool, str | None]:
    required = [
        "qqq_ret_30m",
    ...
    higher_timeframe_trend = (
        float(row["qqq_ret_30m"]) > 0
        and float(row["soxx_ret_30m"]) > 0
        and float(row["qqq_vwap_slope_15m"]) > 0
        and float(row["soxx_vwap_slope_15m"]) > 0
        and float(row["qqq_close"]) > float(row["qqq_vwap"])
        and float(row["soxx_close"]) > float(row["soxx_vwap"])
    )
    ...
    selected = (
        "soxx"
        if float(row["soxx_ret_30m"]) > float(row["qqq_ret_30m"])
        else "qqq"
    )
    execution_symbol = "SOXL" if selected == "soxx" else "TQQQ"

    previous_high = row[f"{selected}_previous_high"]
    if not np.isfinite(previous_high):
        return False, None

    pullback_completed = (
        bool(row[f"{selected}_touch_prior_5m"])
        and bool(
            row[
                f"{selected}_all_closes_above_vwap_prior_5m"
            ]
        )
    )
    reentry_breakout = (
        float(row[f"{selected}_close"]) > float(previous_high)
        and float(row[f"{selected}_close"])
        > float(row[f"{selected}_vwap"])
    )

    return (
        bool(pullback_completed and reentry_breakout),
        execution_symbol
        if pullback_completed and reentry_breakout
        else None,
    )


def state_transition_candidates(
    ...
    for trade_date, day in aligned.groupby("trade_date", sort=True):
        ...
        for timestamp_utc, row in day.sort_index().iterrows():
            current, branch = pullback_reentry_condition_and_branch(row)
            if current and not previous:
                signal_et = pd.Timestamp(row["signal_timestamp_et"])
                selected = "soxx" if branch == "SOXL" else "qqq"
                rows.append(
                    {
                        ...
                        "weekday": int(signal_et.weekday()),
                        "signal_minute_et": int(
                            signal_et.hour * 60 + signal_et.minute
                        ),
                        "execution_symbol": branch,
                        "selected_signal_asset": selected.upper(),
                        "qqq_ret_15m": float(row["qqq_ret_15m"]),
                        "soxx_ret_15m": float(row["soxx_ret_15m"]),
                        "qqq_ret_30m": float(row["qqq_ret_30m"]),
                        "soxx_ret_30m": float(row["soxx_ret_30m"]),
                        "qqq_vwap_slope_15m": float(
                            row["qqq_vwap_slope_15m"]
                        ),
                        "soxx_vwap_slope_15m": float(
                            row["soxx_vwap_slope_15m"]
                        ),
                        ...
                        * 10_000.0,
                        "selected_touch_prior_5m": bool(
                            row[f"{selected}_touch_prior_5m"]
                        ),
                        "selected_previous_high": float(
                            row[f"{selected}_previous_high"]
                        ),
                    }
                )
            previous = current
...
def build_summaries(trades: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for (symbol, horizon), group in period.groupby(["execution_symbol", "horizon_minutes"], sort=True):
        ...
        final_rows.append({
            ...
            "candidate_qualified_for_v22_054": qualified,
        })
...
def run_study(v22_052_summary_path: Path, canonical_root: Path, result_dir: Path) -> dict[str, Any]:
    if not v22_052_summary_path.exists():
        raise StudyError(f"Missing V22.052 summary: {v22_052_summary_path}")
    v22_052 = json.loads(v22_052_summary_path.read_text(encoding="utf-8-sig"))
    validate_v22_052(v22_052)
    ...
    qualified = final.loc[final["candidate_qualified_for_v22_054"]]
    ready = not qualified.empty

    result_dir.mkdir(parents=True, exist_ok=True)
    outputs = {
        "trades_parquet": result_dir / "v22_053_trades.parquet",
        "trades_csv": result_dir / "v22_053_trades.csv",
        "daily": result_dir / "v22_053_daily_returns.csv",
        "period": result_dir / "v22_053_candidate_period_summary.csv",
        "bootstrap": result_dir / "v22_053_block_bootstrap_summary.csv",
        "final": result_dir / "v22_053_final_candidate_table.csv",
        "year": result_dir / "v22_053_year_summary.csv",
        "bucket": result_dir / "v22_053_time_bucket_summary.csv",
        "baseline": result_dir / "v22_053_matched_unconditional_baseline.csv",
        "summary": result_dir / "v22_053_summary.json",
        "manifest": result_dir / "v22_053_run_manifest.json",
    }
    ...
    summary = {
        "version": VERSION,
        "final_status": "PASS",
        "final_decision": "PULLBACK_REENTRY_CANDIDATE_QUALIFIED_FOR_V22_054" if ready else "NO_PULLBACK_REENTRY_CANDIDATE_QUALIFIED",
        "v22_052_validated": True,
        "v22_052_summary_path": str(v22_052_summary_path),
        "v22_052_summary_sha256": sha256_file(v22_052_summary_path),
        ...
        "new_architecture": "TREND_PULLBACK_VWAP_TOUCH_REENTRY_BREAKOUT",
        ...
        "data_ready_for_v22_054": bool(ready),
        "entry_filter_optimization_executed": False,
        "exit_optimization_executed": False,
        "broker_action_allowed": False,
        "paper_trading_allowed": False,
...
def print_final(result: Mapping[str, Any]) -> None:
    ...
    print("NEW_ARCHITECTURE=TREND_PULLBACK_VWAP_TOUCH_REENTRY_BREAKOUT")
    ...
    print(f"DATA_READY_FOR_V22_054={s['data_ready_for_v22_054']}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--v22-052-summary", dest="v22_052_summary", default=r"D:\us-tech-quant-results\v22\V22.052_FAST3_LONG_NONOVERLAP_STATE_TRANSITION_STUDY_R1\v22_052_summary.json")
    parser.add_argument("--canonical-root", default=r"D:\us-tech-quant-data\fast3\moomoo_24h_1m\canonical")
    parser.add_argument("--result-dir", default=r"D:\us-tech-quant-results\v22\V22.053_FAST3_TREND_PULLBACK_VWAP_REENTRY_STUDY_R1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.execute:
        ...
        return 2
    try:
        result = run_study(Path(args.v22_052_summary), Path(args.canonical_root), Path(args.result_dir))
        print_final(result)
        return 0
