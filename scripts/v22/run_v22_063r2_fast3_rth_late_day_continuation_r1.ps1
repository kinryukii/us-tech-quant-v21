...
$Script = ".\scripts\v22\v22_063r2_fast3_rth_late_day_continuation_r1.py"
$Test = ".\scripts\v22\test_v22_063r2_fast3_rth_late_day_continuation_r1.py"
...
$ResultDir = "D:\us-tech-quant-results\v22\V22.063R2_FAST3_RTH_LATE_DAY_CONTINUATION_BASELINE_R1"
$SummaryFile = Join-Path $ResultDir "v22_063r2_summary.json"

$Running = Get-CimInstance Win32_Process |
    Where-Object {
        $_.ProcessId -ne $PID -and
        $_.CommandLine -match "v22_063r2_fast3_rth_late_day_continuation"
    }
...
if ($Running) {
    $Running |
        ...
        Format-List
    throw "V22.063R2 is already running."
}
...
if (Test-Path $SummaryFile) {
    try {
        ...
        if ($Existing.final_status -eq "PASS") {
            Write-Host "V22.063R2 already completed. Existing PASS result is preserved." -ForegroundColor Green
...
if (Test-Path $ResultDir) {
    ...
    Write-Host "Removed incomplete V22.063R2 result directory." -ForegroundColor Yellow
}

$TempRoot = "D:\us-tech-quant-results\pytest_temp"
$RunStamp = Get-Date -Format "yyyyMMdd_HHmmss_fff"
$PytestBase = Join-Path $TempRoot "v22_063r2_$RunStamp"
...
Write-Host " V22.063R2 RTH late-day continuation"
Write-Host "==============================================" -ForegroundColor Cyan
...
Write-Host "TEST_COUNT=20" -ForegroundColor Green

Write-Host ""
Write-Host "Step 3/3: Frozen RTH late-day baseline" -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    throw "V22.063R2 study failed: $LASTEXITCODE"
}
