...
DIRECTION_SCOPE_CONTRACT_VERSION = "V22.042_R2A_EXPLICIT_DIRECTION_SCOPE_CONTRACT"
REASON_FIELDS = ["reason_rank", "reason_code", "reason_detail", "official_gate_blocking", "shadow_relevant"]
MODE_FIELDS = ["gate_mode", "direction_label", "direction_scope", "direction_scope_source", "direction_scope_policy_reason", "wait_state", "candidate_count", "reason_code", "official_gate", "shadow_only", "broker_action_allowed", "official_adoption_allowed", "research_only"]
...
SUMMARY_FIELDS = [
    ...
    "strict_official_promoted_candidate_count", "strict_official_direction_scope", "semiconductor_only_shadow_direction_label",
    "semiconductor_only_shadow_candidate_count", "semiconductor_only_shadow_direction_scope", "relaxed_broad_shadow_direction_label",
    "relaxed_broad_shadow_candidate_count", "relaxed_broad_shadow_direction_scope", "direction_scope_contract_version", "primary_wait_reason_code",
...
def direction_scope_contract(gate_mode: str, direction_label: str) -> tuple[str, str, str]:
    """Return the explicit tradable-universe scope contract for a gate mode."""
    mode = str(gate_mode or "").strip()
    label = str(direction_label or "").strip().upper()

    if mode == "semiconductor_only_shadow_gate":
        return (
            "SEMICONDUCTOR",
            "EXPLICIT_GATE_POLICY",
            "SEMICONDUCTOR_ONLY_GATE_TRADABLE_UNIVERSE",
        )

    if mode == "relaxed_broad_shadow_gate":
        return (
            "SEMICONDUCTOR",
            "EXPLICIT_GATE_POLICY",
    ...
    if "SEMICONDUCTOR" in label or "SEMI" in label:
        scope = "SEMICONDUCTOR"
    elif (
        not label
        or label in {"WAIT", "MIXED", "MIXED_OR_WAIT", "NEUTRAL", "UNKNOWN", "NONE"}
        or "INSUFFICIENT" in label
    ):
        scope = "ALL"
    else:
        scope = "BROAD"
    ...
    return (
        scope,
        "EXPLICIT_GATE_POLICY",
        "STRICT_SCOPE_DERIVED_FROM_FINAL_DIRECTION_LABEL",
    )
...
def run(
    ...
    strict_scope, strict_scope_source, strict_scope_reason = direction_scope_contract(
        "strict_official_gate", strict
    )
    semi_scope, semi_scope_source, semi_scope_reason = direction_scope_contract(
        "semiconductor_only_shadow_gate", semi
    )
    relaxed_scope, relaxed_scope_source, relaxed_scope_reason = direction_scope_contract(
        "relaxed_broad_shadow_gate", relaxed
    )
    ...
    summary = {
        ...
        "strict_official_direction_scope": strict_scope,
        "semiconductor_only_shadow_direction_label": semi,
        "semiconductor_only_shadow_candidate_count": len(semi_promoted),
        "semiconductor_only_shadow_direction_scope": semi_scope,
        "relaxed_broad_shadow_direction_label": relaxed,
        "relaxed_broad_shadow_candidate_count": len(relaxed_promoted),
        "relaxed_broad_shadow_direction_scope": relaxed_scope,
        "direction_scope_contract_version": DIRECTION_SCOPE_CONTRACT_VERSION,
        ...
        "direction_timestamp_contract_version": str(v22_042_summary.get("direction_timestamp_contract_version") or ""),
        "direction_source_time_utc": str(v22_042_summary.get("direction_source_time_utc") or ""),
        "direction_source_time_policy": str(v22_042_summary.get("direction_source_time_policy") or ""),
        "direction_source_time_trust": str(v22_042_summary.get("direction_source_time_trust") or "MISSING"),
        "direction_timestamp_complete": bool(v22_042_summary.get("direction_timestamp_complete", False)),
        "direction_required_timestamp_row_count": int(v22_042_summary.get("direction_required_timestamp_row_count", 0) or 0),
        "direction_parsed_timestamp_row_count": int(v22_042_summary.get("direction_parsed_timestamp_row_count", 0) or 0),
        "direction_input_latest_time_min_utc": str(v22_042_summary.get("direction_input_latest_time_min_utc") or ""),
        "direction_input_latest_time_max_utc": str(v22_042_summary.get("direction_input_latest_time_max_utc") or ""),
        "direction_input_time_dispersion_seconds": v22_042_summary.get("direction_input_time_dispersion_seconds", ""),
        "direction_bar_timestamp_provenance_path": str(v22_042_summary.get("direction_bar_timestamp_provenance_path") or ""),
    ...
    mode_rows = [
        {
            ...
            "direction_label": strict,
            "direction_scope": strict_scope,
            "direction_scope_source": strict_scope_source,
            "direction_scope_policy_reason": strict_scope_reason,
            "wait_state": not bool(strict_promoted),
            "candidate_count": len(strict_promoted),
            "reason_code": primary,
            "official_gate": True,
            "shadow_only": False,
            "broker_action_allowed": False,
        ...
        },
        {
            ...
            "direction_label": semi,
            "direction_scope": semi_scope,
            "direction_scope_source": semi_scope_source,
            "direction_scope_policy_reason": semi_scope_reason,
            "wait_state": not bool(semi_promoted),
            "candidate_count": len(semi_promoted),
            "reason_code": "SHADOW_SEMICONDUCTOR_ONLY_DIRECTION_AVAILABLE" if semi_promoted else primary,
            "official_gate": False,
            "shadow_only": True,
            "broker_action_allowed": False,
            "official_adoption_allowed": False,
        ...
        },
        {
            ...
            "direction_label": relaxed,
            "direction_scope": relaxed_scope,
            "direction_scope_source": relaxed_scope_source,
            "direction_scope_policy_reason": relaxed_scope_reason,
            "wait_state": not bool(relaxed_promoted),
            "candidate_count": len(relaxed_promoted),
            "reason_code": "SHADOW_RELAXED_BROAD_DIRECTION_AVAILABLE" if relaxed_promoted else primary,
            "official_gate": False,
            "shadow_only": True,
            "broker_action_allowed": False,
            "official_adoption_allowed": False,
