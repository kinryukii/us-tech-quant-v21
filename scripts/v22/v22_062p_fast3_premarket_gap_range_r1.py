#!/usr/bin/env python
r"""
V22.062P FAST3 premarket gap-and-range baseline R1.

This stage is structurally independent from:
- the terminated pullback/re-entry architecture;
- the failed RTH synchronized opening-range baseline;
- the failed overnight synchronized trend baseline.

Frozen premarket session
------------------------
- Session: 04:00 through 09:29 ET.
- Initial premarket range: 04:00 through 05:59 ET.
- Signal window: 06:00 through 08:24 ET.
- Maximum one entry per premarket session.
- Entry at the exact next-minute open.
- The signal window ends at 08:24 so that FIXED_60M remains completely
  inside premarket and no exit crosses the 09:30 RTH open.

Frozen prior-close definition
-----------------------------
- Exact 15:59 ET close from the most recent earlier trading date.
- QQQ and SOXX must both have valid prior RTH closes.
- The prior close is strictly earlier than the premarket session date.

Frozen synchronized signal
--------------------------
LONG:
- QQQ and SOXX both trade above their prior RTH closes.
- Both close above their own 04:00-05:59 premarket range highs.
- Both are above cumulative premarket typical-price VWAP.
- Both exact 30-minute returns are positive.
- Both pass the frozen OHLCV tradability proxy.

SHORT:
- QQQ and SOXX both trade below their prior RTH closes.
- Both close below their own initial premarket range lows.
- Both are below cumulative premarket typical-price VWAP.
...
------------------------
For QQQ and SOXX:
- Initial range contains at least 60 observed one-minute bars.
- First observed initial-range bar is no later than 04:10 ET.
- Last observed initial-range bar is no earlier than 05:50 ET.
...
This does not infer bid/ask spread or order-book depth.
...
No hard stop, trailing stop, no-progress exit or factor invalidation.

Diagnostics only
----------------
- Premarket 5-minute RSI(14), MACD(12,26,9), KDJ(9,3,3).
- Prior available official Cboe VIX 1-day change, 3-day change and
  absolute-change percentile.
None of these creates, deletes or sizes a signal.
...
- No gap threshold sweep.
- No liquidity-threshold sweep.
...
VERSION = "V22.062P_FAST3_PREMARKET_GAP_AND_RANGE_BASELINE_R1"
...
class StudyError(RuntimeError):
    """V22.062P cannot continue safely."""
...
def validate_v22_062n(summary: Mapping[str, Any]) -> None:
    ...
    if failures:
        raise StudyError(
            "V22.062N lineage validation failed: " + "; ".join(failures)
        )

    if int(summary.get("canonical_partition_count_indexed", -1)) != 582:
        raise StudyError("V22.062N did not index 582 Canonical partitions")
    if summary.get("supported_exit_variants_for_replication") != []:
        raise StudyError("V22.062N unexpectedly contains a supported exit")
...
def load_premarket_and_close_partition(path: Path) -> pd.DataFrame:
    ...
    premarket_mask = (
        (minute_et >= PREMARKET_START_ET)
        & (minute_et <= PREMARKET_END_ET)
    )
    close_mask = minute_et == RTH_CLOSE_MINUTE_ET
    mask = premarket_mask | close_mask

    selected_et = timestamp_et.loc[mask]
    selected_minute = minute_et.loc[mask].astype(int)
    row_type = np.where(
        selected_minute == RTH_CLOSE_MINUTE_ET,
        "RTH_CLOSE",
        "PREMARKET",
    )
    session_minute = np.where(
        selected_minute == RTH_CLOSE_MINUTE_ET,
        -1,
        selected_minute - PREMARKET_START_ET,
    ).astype(np.int16)
    ...
    frame = pd.DataFrame(
        {
            ...
            "local_date": selected_et.dt.strftime("%Y-%m-%d"),
            "minute_et": selected_minute,
    ...
    if frame["timestamp_utc"].duplicated().any():
        raise StudyError(f"Duplicate selected timestamp: {path}")
    if (
        (frame["low"] > frame["high"]).any()
        or (frame["close"] < frame["low"]).any()
        or (frame["close"] > frame["high"]).any()
    ):
        raise StudyError(f"Invalid selected OHLC relationship: {path}")
    return frame


def load_symbol_premarket(
    symbol: str,
    ...
    frames = [load_premarket_and_close_partition(path) for path in paths]
    ...
    premarket = frame.loc[
        frame["row_type"] == "PREMARKET"
    ].copy()
    closes = frame.loc[
        frame["row_type"] == "RTH_CLOSE"
    ].copy()

    if premarket.duplicated(["local_date", "session_minute"]).any():
        duplicates = premarket.loc[
            premarket.duplicated(
        ...
        raise StudyError(
            f"Cross-partition premarket duplicate for {symbol}: "
            f"{duplicates.to_dict('records')}"
        )

    if closes.duplicated("local_date").any():
        duplicates = closes.loc[
            closes.duplicated("local_date", keep=False),
            ["local_date", "timestamp_utc"],
        ].head()
        raise StudyError(
            f"Duplicate RTH close for {symbol}: "
            f"{duplicates.to_dict('records')}"
        )

    premarket = premarket.set_index(
        ["local_date", "session_minute"],
        drop=False,
    ).sort_index()

    closes = closes[
        ["local_date", "timestamp_utc", "close"]
    ].rename(
        columns={
            "local_date": "prior_close_date",
            "timestamp_utc": "prior_close_timestamp_utc",
            "close": "prior_rth_close",
        }
    ).sort_values(
        "prior_close_date",
        kind="mergesort",
...
def prior_close_for_session(
    closes: pd.DataFrame,
    session_date: str,
) -> dict[str, Any] | None:
    cutoff = pd.Timestamp(session_date)
    eligible = closes.loc[
        closes["prior_close_date_timestamp"] < cutoff
    ]
    if eligible.empty:
        return None
    row = eligible.iloc[-1]
    return {
        "prior_close_date": str(row["prior_close_date"]),
        "prior_close_timestamp_utc": row[
            "prior_close_timestamp_utc"
        ],
...
def build_candidate(
    ...
    qqq_long = (
        ...
        & (qqq["close"] > float(qqq_prior["prior_rth_close"]))
    ...
    soxx_long = (
        ...
        & (soxx["close"] > float(soxx_prior["prior_rth_close"]))
    ...
    candidate: dict[str, Any] = {
        ...
        "qqq_prior_close_date": qqq_prior["prior_close_date"],
        "soxx_prior_close_date": soxx_prior["prior_close_date"],
        "qqq_prior_rth_close": float(qqq_prior["prior_rth_close"]),
        "soxx_prior_rth_close": float(soxx_prior["prior_rth_close"]),
        "qqq_gap_at_signal": (
            float(qqq_row["close"])
...
def exit_minute(entry_minute: int, variant: str) -> int:
    ...
    if variant == "PREMARKET_0925":
        return PREMARKET_EXIT_MINUTE
            ...
            rows.append(
                {
                    ...
                    "mean_qqq_gap_at_signal": float(
                        group["qqq_gap_at_signal"].mean()
                    ),
                    "mean_soxx_gap_at_signal": float(
                        group["soxx_gap_at_signal"].mean()
                    ),
                }
            )
...
def choose_decision(
    ...
    return ("NO_PREMARKET_BASELINE_CANDIDATE_QUALIFIED", [])
...
def format_value(column: str, value: Any) -> str:
    ...
    if column in {
        ...
        "cumulative_return",
        "max_drawdown",
        "mean_qqq_gap_at_signal",
        "mean_soxx_gap_at_signal",
    }:
...
def run_study(
    ...
    summary_062n = json.loads(
        v22_062n_summary_path.read_text(encoding="utf-8-sig")
    )
    ...
    symbol_sessions: dict[str, pd.DataFrame] = {}
    symbol_closes: dict[str, pd.DataFrame] = {}
    for symbol in ALL_SYMBOLS:
        sessions, closes = load_symbol_premarket(
            symbol,
            canonical,
        )
        symbol_sessions[symbol] = sessions
        symbol_closes[symbol] = closes

    session_sets = {
        symbol: set(
            table.index.get_level_values("local_date").unique()
        )
        for symbol, table in symbol_sessions.items()
    }
    ...
    funnel = {
        ...
        "both_prior_closes_available_count": 0,
    ...
    for session_date in common_signal_sessions:
        qqq_prior = prior_close_for_session(
            symbol_closes["QQQ"],
            session_date,
        )
        soxx_prior = prior_close_for_session(
            symbol_closes["SOXX"],
            session_date,
        )
        if qqq_prior is None or soxx_prior is None:
            continue
        funnel["both_prior_closes_available_count"] += 1

        qqq_grid = session_grid(
            get_session(symbol_sessions["QQQ"], session_date)
        )
        soxx_grid = session_grid(
            get_session(symbol_sessions["SOXX"], session_date)
        )
    ...
    if candidates.empty:
        raise StudyError("No premarket candidate was generated")
    if trades.empty:
        raise StudyError("No executable premarket trade was generated")
    ...
    outputs = {
        ...
        / "v22_062p_signal_candidates.csv",
        "trades": result_dir / "v22_062p_trades.csv",
        "daily": result_dir
        / "v22_062p_daily_account_returns.csv",
        "period": result_dir
        / "v22_062p_period_summary.csv",
        "year": result_dir
        / "v22_062p_year_summary.csv",
        "qualification": result_dir
        / "v22_062p_qualification.csv",
        "funnel": result_dir
        / "v22_062p_tradability_funnel.csv",
        "manifest": result_dir
        / "v22_062p_manifest.json",
        "summary": result_dir / "v22_062p_summary.json",
    }
    ...
    summary = {
        ...
        "session_end_et": "09:29",
        "initial_range_start_et": "04:00",
        "initial_range_end_et": "05:59",
        "signal_start_et": "06:00",
        "signal_end_et": "08:24",
        "maximum_entries_per_session": 1,
        "entry_timing": "EXACT_NEXT_MINUTE_OPEN",
        "prior_close_definition": (
            "MOST_RECENT_STRICTLY_EARLIER_EXACT_1559_ET_CLOSE"
        ),
    ...
    manifest = {
        ...
        "inputs": {
            "v22_062n_summary": str(v22_062n_summary_path),
            "v22_062n_summary_sha256": sha256_file(
                v22_062n_summary_path
            ),
    ...
    print("==============================================")
    print(" V22.062P premarket gap-and-range baseline")
    print("==============================================")
    ...
    print("OVERNIGHT_ARCHITECTURE_USED=False")
    ...
    print("GAP_THRESHOLD_SWEEP_EXECUTED=False")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_study(
            v22_062n_summary_path=Path(args.v22_062n_summary),
