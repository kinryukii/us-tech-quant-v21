...
def build_outputs(
    ...
    # V22.042_R2B_EXPLICIT_DIRECTION_MARKET_TIME_ONLY
    # Direction freshness uses explicit V22.042 market-bar time only.
    # Filesystem mtime remains discovery metadata and is never market-data time.
    direction_time = parse_datetime(direction_summary.get("direction_source_time_utc"))
    if direction_time is not None:
        direction_time = (
            direction_time.replace(tzinfo=UTC)
            if direction_time.tzinfo is None
            else direction_time.astimezone(UTC)
        )
    direction_time_trust = str(
        direction_summary.get("direction_source_time_trust") or "MISSING"
    ).strip().upper()
    direction_timestamp_complete = bool_value(
        direction_summary.get("direction_timestamp_complete")
    )
    direction_time_eligible = (
        direction_time is not None
        and direction_timestamp_complete
        and direction_time_trust == "HIGH"
    )

    panel_age_minutes = (now - panel_ref).total_seconds() / 60.0 if panel_ref else math.inf
    direction_panel_gap_minutes = (
        abs((panel_ref - direction_time).total_seconds()) / 60.0
        if panel_ref is not None and direction_time is not None
        else math.inf
    )
    panel_fresh = panel_ref is not None and -5.0 <= panel_age_minutes <= config.max_panel_age_minutes
    direction_fresh = (
        direction_time_eligible
        and panel_ref is not None
        and direction_panel_gap_minutes <= config.max_direction_panel_gap_minutes
    )
    ...
    context = {
        ...
        "direction_source_time_utc": direction_time.isoformat() if direction_time else "",
        "direction_source_time_policy": str(direction_summary.get("direction_source_time_policy") or ""),
        "direction_source_time_trust": direction_time_trust,
        "direction_timestamp_complete": direction_timestamp_complete,
        "direction_time_eligible": direction_time_eligible,
...
def execute(
    ...
    try:
        ...
        provenance_rows = [
            ...
            {"provenance_key": "direction_source_time_utc", "provenance_value": context["direction_source_time_utc"], "trust_level": context["direction_source_time_trust"], "notes": context["direction_source_time_policy"] or "Explicit V22.042 market-bar timestamp required; no filesystem fallback."},
            {"provenance_key": "direction_timestamp_complete", "provenance_value": context["direction_timestamp_complete"], "trust_level": "HIGH" if context["direction_timestamp_complete"] else "MISSING", "notes": "All required SOXX/QQQ[/SPY] timeframe timestamps must parse."},
            {"provenance_key": "direction_time_eligible", "provenance_value": context["direction_time_eligible"], "trust_level": "DERIVED", "notes": "Eligible only when explicit timestamp exists, provenance is complete, and trust is HIGH."},
            {"provenance_key": "direction_panel_time_gap_minutes", "provenance_value": context["direction_panel_time_gap_minutes"], "trust_level": "DERIVED", "notes": "Absolute explicit market-time gap used by stale-direction guard."},
        ...
        policy = {
            ...
            "direction_timestamp_contract": {
                "contract_version": "V22.042_R2B_DIRECTION_BAR_TIMESTAMP_PROVENANCE",
                "required_source": "EXPLICIT_V22_042_MARKET_BAR_TIMESTAMP",
                "accepted_trust": ["HIGH"],
                "requires_complete_provenance": True,
                "filesystem_mtime_allowed_as_market_time": False,
                "missing_or_untrusted_behavior": "FAIL_CLOSED_NO_TRADE",
            },
            "direction_mapping": {
        ...
        summary: dict[str, Any] = {
            ...
            "direction_source_time_policy": context["direction_source_time_policy"],
            "direction_source_time_trust": context["direction_source_time_trust"],
            "direction_timestamp_complete": context["direction_timestamp_complete"],
            "direction_time_eligible": context["direction_time_eligible"],
