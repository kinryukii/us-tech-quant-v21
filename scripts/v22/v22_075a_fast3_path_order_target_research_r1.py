...
REPO=Path(__file__).resolve().parents[2]; OUT_NAME='V22.075A_FAST3_PATH_ORDER_TARGET_RESEARCH_R1'; DATA_ROOT=Path(r'D:/us-tech-quant-data/fast3/moomoo_24h_1m')
CONTRACT=REPO/'.local_results/v22/V22.073A_FAST3_PIT_FEATURE_EXPANSION_RESEARCH_R1/feature_contract.json'; SYMS=['QQQ','SOXX','TQQQ','SQQQ','SOXL','SOXS']; COST=.001; TP=.030; SL=-.006
...
def run(results_root=REPO/'.local_results'):
 ...
 out=Path(results_root)/'v22'/OUT_NAME;out.parent.mkdir(parents=True,exist_ok=True);stage=Path(tempfile.mkdtemp(prefix='.075a_',dir=out.parent))
 try:
