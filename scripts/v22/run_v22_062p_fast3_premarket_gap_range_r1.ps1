...
$Script = ".\scripts\v22\v22_062p_fast3_premarket_gap_range_r1.py"
$Test = ".\scripts\v22\test_v22_062p_fast3_premarket_gap_range_r1.py"
...
$ResultDir = "D:\us-tech-quant-results\v22\V22.062P_FAST3_PREMARKET_GAP_AND_RANGE_BASELINE_R1"
$SummaryFile = Join-Path $ResultDir "v22_062p_summary.json"

$Running = Get-CimInstance Win32_Process |
    Where-Object {
        $_.ProcessId -ne $PID -and
        $_.CommandLine -match "v22_062p_fast3_premarket_gap_range"
    }
...
if ($Running) {
    $Running |
        ...
        Format-List
    throw "V22.062P is already running."
}
...
if (Test-Path $SummaryFile) {
    try {
        ...
        if ($Existing.final_status -eq "PASS") {
            Write-Host "V22.062P already completed. Existing PASS result is preserved." -ForegroundColor Green
        ...
        Write-Host "Existing V22.062P summary is invalid; incomplete results will be replaced." -ForegroundColor Yellow
    }
}

if (Test-Path $ResultDir) {
    Remove-Item $ResultDir -Recurse -Force
    Write-Host "Removed incomplete V22.062P result directory." -ForegroundColor Yellow
}

$TempRoot = "D:\us-tech-quant-results\pytest_temp"
$RunStamp = Get-Date -Format "yyyyMMdd_HHmmss_fff"
$PytestBase = Join-Path $TempRoot "v22_062p_$RunStamp"
...
Write-Host " V22.062P premarket gap-and-range baseline"
Write-Host "==============================================" -ForegroundColor Cyan
...
Write-Host "TEST_COUNT=30" -ForegroundColor Green

Write-Host ""
Write-Host "Step 3/3: Frozen premarket baseline" -ForegroundColor Yellow
...
Write-Host "The process may use substantial RAM while holding premarket rows." -ForegroundColor Yellow
Write-Host "Long quiet periods are normal. Do not press Ctrl+C." -ForegroundColor Yellow

& $Python $Script --execute
if ($LASTEXITCODE -ne 0) {
    throw "V22.062P study failed: $LASTEXITCODE"
}
