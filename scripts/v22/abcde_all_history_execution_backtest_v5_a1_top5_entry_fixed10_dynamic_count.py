...
def run_a1_top5_entry_fixed10_dynamic_count(
    rankings: pd.DataFrame,
    prices: pd.DataFrame,
    sessions: pd.DatetimeIndex,
    cost_bps: float,
    ...
    """A1 dynamic-count policy requested by the user.

    Rules:
    - Initial signal: buy all current A1 Top 5 names, each using 10% of initial NAV.
      Initial gross exposure is therefore approximately 50%.
    - Afterwards, whenever a ticker is in the current A1 Top 5 and is not already
      held, buy it using 10% of initial NAV.
    - Existing holdings are retained while their current A1 rank is within Top 10.
    - Sell the entire position once it falls outside A1 Top 10 or disappears.
    - No daily rebalancing and no five-position cap.
    - Cash is the only funding constraint; if insufficient cash is available,
      the new entry is skipped and recorded in the missing/audit output.
    """
    name = "A1_TOP5_ENTRY_FIXED10_EXIT10_DYNAMIC_COUNT"
    initial_nav = 1.0
    fixed_entry_notional = 0.10 * initial_nav
    cost_rate = float(cost_bps) / 10_000.0
    ...
    for date in sessions[(sessions >= start_date) & (sessions <= end_date)]:
        ...
        if date in exec_signals:
            ...
            # 1) Exit anything outside Top 10.
            exits = [ticker for ticker in list(shares) if ticker not in top10]
            for ticker in exits:
                op = get_price(date, ticker, "open", fallback_prior=True)
                if op is None:
                ...
                trade_rows.append({
                    ...
                    "cost_allocated": cost,
                    "a1_rank": rank_pos.get(ticker),
                    "rule": "FULL_EXIT_OUTSIDE_TOP10",
                })

            # 2) Buy every current Top-5 name not already held.
            # Each entry always uses 10% of INITIAL NAV.
            for ticker in top5:
                if ticker in shares:
                    continue
                ...
                if cash + 1e-12 < required_cash:
                    missing_rows.append({
                        ...
                        "required_cash": required_cash,
                        "available_cash": cash,
                        "current_position_count": len(shares),
                    })
                    continue
                ...
                trade_rows.append({
                    ...
                    "cost_allocated": cost,
                    "a1_rank": rank_pos.get(ticker),
                    "rule": "BUY_NEW_CURRENT_TOP5_FIXED_10PCT_INITIAL_NAV",
                })
                if active_start is None:
                    active_start = date
            ...
            # Actual post-trade open weights.
            post_trade_nav_open = cash
            open_values: Dict[str, float] = {}
            for ticker, qty in shares.items():
                op = get_price(date, ticker, "open", fallback_prior=True)
    ...
    summary = {
        ...
        "average_gross_exposure": float(nav["gross_exposure_pct"].mean()),
        "average_position_count": float(nav["position_count"].mean()),
        "max_position_count": int(nav["position_count"].max()),
        "fixed_entry_fraction_initial_nav": 0.10,
        "routine_rebalancing": False,
        "position_cap": None,
    }
...
def main() -> int:
    ...
    # Corrected dynamic-count version:
    # start at 50% (five positions x 10%), then buy every new Top-5 entrant,
    # retain all holdings while in Top 10, and sell only outside Top 10.
    dynamic_result = run_a1_top5_entry_fixed10_dynamic_count(
        rankings=rankings,
        prices=prices,
        sessions=sessions,
        cost_bps=args.cost_bps,
    ...
    summaries.append(dynamic_result.summary)
    navs.append(dynamic_result.nav)
    if not dynamic_result.trades.empty:
        trades.append(dynamic_result.trades)
    if not dynamic_result.target_audit.empty:
        targets.append(dynamic_result.target_audit)
    if not dynamic_result.missing.empty:
        missing.append(dynamic_result.missing)
