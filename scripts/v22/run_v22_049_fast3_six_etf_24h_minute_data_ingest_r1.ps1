...
$exe=if($env:APPDATA){Join-Path $env:APPDATA 'moomoo_OpenD\moomoo_OpenD.exe'}else{$null}; if($exe -and (Test-Path $exe)){$env:MOOMOO_OPEND_VERSION=(Get-Item $exe).VersionInfo.ProductVersion}
...
$summary='D:\us-tech-quant-results\runtime\fast3\V22.049_FAST3_SIX_ETF_24H_MINUTE_DATA_INGEST_R1\v22_049_summary.json'; if(-not(Test-Path $summary)){throw "Summary missing: $summary"}; Get-Content $summary -Raw; exit 0
