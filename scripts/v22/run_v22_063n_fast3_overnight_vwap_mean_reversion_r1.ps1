...
$Base = ".\scripts\v22\v22_063r1_fast3_rth_vwap_mean_reversion_r1.py"
$Script = ".\scripts\v22\v22_063n_fast3_overnight_vwap_mean_reversion_r1.py"
$Test = ".\scripts\v22\test_v22_063n_fast3_overnight_vwap_mean_reversion_r1.py"

foreach ($File in @($Python, $Base, $Script, $Test)) {
    if (-not (Test-Path $File)) {
        throw "Missing required file: $File"
    }
}

$ResultDir = "D:\us-tech-quant-results\v22\V22.063N_FAST3_OVERNIGHT_VWAP_MEAN_REVERSION_BASELINE_R1"
$SummaryFile = Join-Path $ResultDir "v22_063n_summary.json"

$Running = Get-CimInstance Win32_Process |
    Where-Object {
        $_.ProcessId -ne $PID -and
        $_.CommandLine -match "v22_063n_fast3_overnight_vwap_mean_reversion"
    }
if ($Running) {
    $Running | Select-Object ProcessId, CreationDate, CommandLine | Format-List
    throw "V22.063N is already running."
}
...
if (Test-Path $SummaryFile) {
    try {
        ...
        if ($Existing.final_status -eq "PASS") {
            Write-Host "V22.063N already completed. Existing PASS result is preserved." -ForegroundColor Green
        ...
        Write-Host "Existing summary is invalid; incomplete results will be replaced." -ForegroundColor Yellow
    }
}

if (Test-Path $ResultDir) {
    Remove-Item $ResultDir -Recurse -Force
    Write-Host "Removed incomplete V22.063N result directory." -ForegroundColor Yellow
}

$TempRoot = "D:\us-tech-quant-results\pytest_temp"
$RunStamp = Get-Date -Format "yyyyMMdd_HHmmss_fff"
$PytestBase = Join-Path $TempRoot "v22_063n_$RunStamp"
...
Write-Host " V22.063N overnight VWAP mean-reversion baseline"
Write-Host "==============================================" -ForegroundColor Cyan

Write-Host ""
Write-Host "Step 1/3: Python compile" -ForegroundColor Yellow
& $Python -m py_compile $Base $Script $Test
...
Write-Host ""
Write-Host "Step 3/3: Frozen overnight mean-reversion baseline" -ForegroundColor Yellow
...
Write-Host "The process may use substantial RAM." -ForegroundColor Yellow
Write-Host "Long quiet periods are normal. Do not press Ctrl+C." -ForegroundColor Yellow

& $Python $Script --execute
if ($LASTEXITCODE -ne 0) { throw "V22.063N study failed: $LASTEXITCODE" }
