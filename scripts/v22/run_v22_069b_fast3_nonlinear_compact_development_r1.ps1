...
if($Execute){& $p $m --execute;$code=$LASTEXITCODE;if($code -ne 0){exit $code};$summary=Get-Content -Raw 'D:\us-tech-quant-results\fast3\archive\legacy_v22\V22.069B_FAST3_NONLINEAR_COMPACT_DEVELOPMENT_R1\summary.json'|ConvertFrom-Json;if($summary.final_status -ne 'PASS'){exit 1}}
