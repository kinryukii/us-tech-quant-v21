...
$Script = ".\scripts\v22\v22_061_fast3_synchronized_opening_range_breakout_r1.py"
$Test = ".\scripts\v22\test_v22_061_fast3_synchronized_opening_range_breakout_r1.py"
...
$ResultDir = "D:\us-tech-quant-results\v22\V22.061_FAST3_SYNCHRONIZED_OPENING_RANGE_BREAKOUT_BASELINE_R1"
$SummaryFile = Join-Path $ResultDir "v22_061_summary.json"

$Running = Get-CimInstance Win32_Process |
    Where-Object {
        $_.ProcessId -ne $PID -and
        $_.CommandLine -match "v22_061_fast3_synchronized_opening_range_breakout"
    }
...
if ($Running) {
    $Running |
        ...
        Format-List
    throw "V22.061 is already running."
}
...
if (Test-Path $SummaryFile) {
    try {
        ...
        if ($Existing.final_status -eq "PASS") {
            Write-Host "V22.061 already completed. Existing PASS result is preserved." -ForegroundColor Green
        ...
        Write-Host "Existing V22.061 summary is invalid; incomplete results will be replaced." -ForegroundColor Yellow
    }
}

if (Test-Path $ResultDir) {
    Remove-Item $ResultDir -Recurse -Force
    Write-Host "Removed incomplete V22.061 result directory." -ForegroundColor Yellow
}

$TempRoot = "D:\us-tech-quant-results\pytest_temp"
$RunStamp = Get-Date -Format "yyyyMMdd_HHmmss_fff"
$PytestBase = Join-Path $TempRoot "v22_061_$RunStamp"
...
Write-Host " V22.061 synchronized opening-range breakout"
Write-Host "==============================================" -ForegroundColor Cyan
...
Write-Host "TEST_COUNT=29" -ForegroundColor Green

Write-Host ""
Write-Host "Step 3/3: Frozen synchronized ORB baseline" -ForegroundColor Yellow
Write-Host "This reads the existing 582 Canonical partitions." -ForegroundColor Yellow
Write-Host "It creates no new market-data cache and downloads nothing." -ForegroundColor Yellow
Write-Host "Long quiet periods are normal. Do not press Ctrl+C." -ForegroundColor Yellow

& $Python $Script --execute
if ($LASTEXITCODE -ne 0) {
    throw "V22.061 study failed: $LASTEXITCODE"
}
