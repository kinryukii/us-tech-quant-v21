#!/usr/bin/env python
r"""V22.058 FAST3 KDJ temporal-alignment study R1.

Frozen diagnostic architecture:
- Prior-day official Cboe VIX regime.
- QQQ/SOXX 5-minute RSI, MACD, KDJ and 30-minute relative strength.
- Compare strict same-bar KDJ cross, recent-three-bar KDJ state, and no-KDJ control.
- All non-KDJ entry rules, execution, costs, sizing and dynamic exits are identical.
...
VERSION = "V22.058_FAST3_KDJ_TEMPORAL_ALIGNMENT_STUDY_R1"
...
def validate_v22_057(summary: Mapping[str, Any]) -> None:
    expected = {
        "final_status": "PASS",
    ...
    if failures:
        raise StudyError("V22.057 validation failed: " + "; ".join(failures))
...
def build_candidates(aligned: pd.DataFrame, vix: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    funnel = {
        ...
        "long_price": 0,
        "short_price": 0,
        "strict_long_signal": 0,
        "strict_short_signal": 0,
        "recent3_long_signal": 0,
        "recent3_short_signal": 0,
        "no_kdj_long_signal": 0,
        "no_kdj_short_signal": 0,
    }
    for timestamp, row in work.iterrows():
        ...
        for direction in ("LONG", "SHORT"):
            ...
            if gates["kdj_strict"]:
                funnel[f"{direction.lower()}_kdj_strict"] += 1
            if gates["kdj_recent3"]:
                funnel[f"{direction.lower()}_kdj_recent3"] += 1
            if gates["price"]:
            ...
            base_without_kdj = trend and gates["rsi"] and gates["price"] and vix_allowed
            strict_signal = base_without_kdj and gates["kdj_strict"]
            recent3_signal = base_without_kdj and gates["kdj_recent3"]
            no_kdj_signal = base_without_kdj
            if strict_signal:
                funnel[f"strict_{direction.lower()}_signal"] += 1
            if recent3_signal:
                funnel[f"recent3_{direction.lower()}_signal"] += 1
            if no_kdj_signal:
                funnel[f"no_kdj_{direction.lower()}_signal"] += 1
            if not (strict_signal or recent3_signal or no_kdj_signal):
                continue
            atr_value = row.get(f"{execution_symbol.lower()}_atr_pct", np.nan)
            cross_age_column = (
                f"{prefix}_kdj_long_cross_age3"
                if direction == "LONG"
                else f"{prefix}_kdj_short_cross_age3"
            )
            cross_age = row.get(cross_age_column, np.nan)
            rows.append(
                {
                    ...
                    "strict_same_bar_kdj_signal": bool(strict_signal),
                    "recent_3bar_kdj_signal": bool(recent3_signal),
                    "no_kdj_control_signal": bool(no_kdj_signal),
                    "kdj_cross_age_bars": float(cross_age) if np.isfinite(cross_age) else np.nan,
...
def candidate_mask(candidates: pd.DataFrame, variant: str) -> pd.Series:
    if variant == "STRICT_SAME_BAR_KDJ":
        return candidates["strict_same_bar_kdj_signal"]
    if variant == "RECENT_3BAR_KDJ":
        return candidates["recent_3bar_kdj_signal"]
    if variant == "NO_KDJ_CONTROL":
        return candidates["no_kdj_control_signal"]
...
def simulate_one_trade(
    ...
    fixed_control = False
    include_kdj_exit = variant != "NO_KDJ_CONTROL"
...
def qualification(period: pd.DataFrame, year: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    period_lookup: dict[tuple[str, str], pd.Series] = {}
    for _, row in period.iterrows():
        period_lookup[(str(row["variant"]), str(row["study_period"]))] = row

    for variant in VARIANTS:
        development = period_lookup.get((variant, "2018-2022_DEVELOPMENT"))
        validation = period_lookup.get((variant, "2023-2024_VALIDATION"))
        confirmation = period_lookup.get((variant, "2025-2026_YTD_CONFIRMATION"))
        if development is None or validation is None or confirmation is None:
            continue
        full_history_trade_count = int(
            development["trade_count"]
            + validation["trade_count"]
            + confirmation["trade_count"]
        )
        sample_evaluable = bool(
            full_history_trade_count >= MIN_FULL_HISTORY_TRADES
            and validation["trade_count"] >= MIN_VALIDATION_TRADES
            and confirmation["trade_count"] >= MIN_CONFIRMATION_TRADES
        )
        validation_mean_positive = bool(
            validation["mean_instrument_net_return"] > 0
        )
        confirmation_mean_positive = bool(
            confirmation["mean_instrument_net_return"] > 0
        )
        validation_pit_positive = bool(
            validation["mean_entry_excess_pit_60m"] > 0
        )
        confirmation_pit_positive = bool(
            confirmation["mean_entry_excess_pit_60m"] > 0
        )
        research_candidate = bool(
            sample_evaluable
            and validation_mean_positive
            and confirmation_mean_positive
            and validation_pit_positive
            and confirmation_pit_positive
        )
        rows.append(
            {
                "variant": variant,
                "full_history_trade_count": full_history_trade_count,
                "full_history_trade_count_pass": bool(
                    full_history_trade_count >= MIN_FULL_HISTORY_TRADES
                ),
                "validation_trade_count": int(validation["trade_count"]),
                "validation_trade_count_pass": bool(
                    validation["trade_count"] >= MIN_VALIDATION_TRADES
                ),
                "confirmation_trade_count": int(confirmation["trade_count"]),
                "confirmation_trade_count_pass": bool(
                    confirmation["trade_count"] >= MIN_CONFIRMATION_TRADES
                ),
                "sample_evaluable": sample_evaluable,
                "validation_mean_net_return": float(
                    validation["mean_instrument_net_return"]
                ),
                "confirmation_mean_net_return": float(
                    confirmation["mean_instrument_net_return"]
                ),
                "validation_median_net_return": float(
                    validation["median_instrument_net_return"]
                ),
                "confirmation_median_net_return": float(
                    confirmation["median_instrument_net_return"]
                ),
                "validation_profit_factor": float(validation["profit_factor"]),
                "confirmation_profit_factor": float(confirmation["profit_factor"]),
                "validation_mean_excess_pit_60m": float(
                    validation["mean_entry_excess_pit_60m"]
                ),
                "confirmation_mean_excess_pit_60m": float(
                    confirmation["mean_entry_excess_pit_60m"]
                ),
                "validation_mean_positive": validation_mean_positive,
                "confirmation_mean_positive": confirmation_mean_positive,
                "validation_pit_positive": validation_pit_positive,
                "confirmation_pit_positive": confirmation_pit_positive,
                "direction_positive_both_periods": bool(
                    validation_mean_positive and confirmation_mean_positive
                ),
                "pit_excess_positive_both_periods": bool(
                    validation_pit_positive and confirmation_pit_positive
                ),
                "single_year_profit_share": single_year_profit_share(year, variant),
                "research_candidate_for_next_stage": research_candidate,
            }
        )
    ...
    no_kdj = result.loc[result["variant"] == "NO_KDJ_CONTROL"]
    recent = result.loc[result["variant"] == "RECENT_3BAR_KDJ"]
    if len(no_kdj) and len(recent):
        no_kdj_row = no_kdj.iloc[0]
        recent_index = recent.index[0]
        result.loc[recent_index, "recent3_beats_no_kdj_validation_mean"] = bool(
            result.loc[recent_index, "validation_mean_net_return"]
            > no_kdj_row["validation_mean_net_return"]
        )
        result.loc[recent_index, "recent3_beats_no_kdj_confirmation_mean"] = bool(
            result.loc[recent_index, "confirmation_mean_net_return"]
            > no_kdj_row["confirmation_mean_net_return"]
        )
        result.loc[recent_index, "kdj_incremental_mean_support"] = bool(
            result.loc[recent_index, "recent3_beats_no_kdj_validation_mean"]
            and result.loc[recent_index, "recent3_beats_no_kdj_confirmation_mean"]
        )
    return result


def choose_final_decision(qualified: pd.DataFrame) -> tuple[str, bool]:
    if qualified.empty:
        return "NO_KDJ_TEMPORAL_ALIGNMENT_CANDIDATE_QUALIFIED", False
    recent = qualified.loc[qualified["variant"] == "RECENT_3BAR_KDJ"]
    no_kdj = qualified.loc[qualified["variant"] == "NO_KDJ_CONTROL"]
    if recent.empty:
        return "NO_KDJ_TEMPORAL_ALIGNMENT_CANDIDATE_QUALIFIED", False
    recent_row = recent.iloc[0]
    no_kdj_candidate = bool(
        len(no_kdj)
        and no_kdj.iloc[0]["research_candidate_for_next_stage"]
    )
    if not bool(recent_row["sample_evaluable"]):
        return "RECENT_3BAR_KDJ_INCONCLUSIVE_INSUFFICIENT_SAMPLE", False
    if bool(recent_row["research_candidate_for_next_stage"]):
        incremental = recent_row.get("kdj_incremental_mean_support", False)
        if bool(incremental) or not no_kdj_candidate:
            return "RECENT_3BAR_KDJ_SUPPORTED_FOR_INDEPENDENT_REPLICATION", True
        return "KDJ_NOT_INCREMENTAL_USE_NO_KDJ_CORE", False
    if no_kdj_candidate:
        return "KDJ_HARD_GATE_NOT_SUPPORTED_DEMOTE_TO_DIAGNOSTIC", False
    return "NO_KDJ_TEMPORAL_ALIGNMENT_CANDIDATE_QUALIFIED", False


def run_study(
    v22_057_summary_path: Path,
    v22_056_summary_path: Path,
    vix_feature_path: Path,
    canonical_root: Path,
    result_dir: Path,
) -> dict[str, Any]:
    if not v22_057_summary_path.exists():
        raise StudyError(f"V22.057 summary not found: {v22_057_summary_path}")
    v22_057 = json.loads(v22_057_summary_path.read_text(encoding="utf-8-sig"))
    validate_v22_057(v22_057)
    ...
    final_decision, recent3_supported = choose_final_decision(qualified)

    result_dir.mkdir(parents=True, exist_ok=True)
    outputs = {
        "candidates_csv": result_dir / "v22_058_signal_candidates.csv",
        "trades_csv": result_dir / "v22_058_trades.csv",
        "trades_parquet": result_dir / "v22_058_trades.parquet",
        "daily_nav": result_dir / "v22_058_daily_nav.csv",
        "period_summary": result_dir / "v22_058_period_summary.csv",
        "year_summary": result_dir / "v22_058_year_summary.csv",
        "exit_summary": result_dir / "v22_058_exit_reason_summary.csv",
        "direction_summary": result_dir / "v22_058_direction_symbol_summary.csv",
        "qualification": result_dir / "v22_058_qualification_table.csv",
        "signal_funnel": result_dir / "v22_058_signal_funnel.csv",
        "summary": result_dir / "v22_058_summary.json",
        "manifest": result_dir / "v22_058_run_manifest.json",
    }
    ...
    summary = {
        ...
        "recent3_candidate_retention_vs_no_kdj": (
            float(candidate_mask(candidates, "RECENT_3BAR_KDJ").sum())
            / float(candidate_mask(candidates, "NO_KDJ_CONTROL").sum())
            if not candidates.empty and candidate_mask(candidates, "NO_KDJ_CONTROL").sum() > 0
            else math.nan
        ),
        "recent3_kdj_supported_for_replication": recent3_supported,
        "kdj_temporal_alignment_executed": True,
        "kdj_recent_bars": KDJ_RECENT_BARS,
        ...
        "paper_trading_allowed": False,
        "official_adoption_allowed": False,
        "v22_057_summary_sha256": sha256_file(v22_057_summary_path),
    ...
    manifest = {
        ...
        "inputs": {
            "v22_057_summary": str(v22_057_summary_path),
...
def print_final(result: Mapping[str, Any]) -> None:
    ...
    print(f"RECENT3_CANDIDATE_RETENTION_VS_NO_KDJ={summary['recent3_candidate_retention_vs_no_kdj']}")
    ...
    print(f"RECENT3_KDJ_SUPPORTED_FOR_REPLICATION={summary['recent3_kdj_supported_for_replication']}")
    print("KDJ_TEMPORAL_ALIGNMENT_EXECUTED=True")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_study(
            v22_057_summary_path=Path(args.v22_057_summary),
