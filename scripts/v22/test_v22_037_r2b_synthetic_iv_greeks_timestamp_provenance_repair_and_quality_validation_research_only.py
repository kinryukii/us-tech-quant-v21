...
MODULE_PATH = Path(__file__).with_name(
    "v22_037_r2b_synthetic_iv_greeks_timestamp_provenance_repair_and_quality_validation_research_only.py"
)
SPEC = importlib.util.spec_from_file_location("v22_037_r2b", MODULE_PATH)
...
def test_end_to_end_execute_passes_and_writes_all_outputs(tmp_path: Path) -> None:
    ...
    for filename in (
        ...
        "V22.037_R2B_synthetic_iv_greeks_timestamp_provenance_repair_and_quality_validation_report.txt",
    ):
        assert (output_dir / filename).exists(), filename
    loaded = json.loads((output_dir / "v22_037_r2b_summary.json").read_text(encoding="utf-8"))
...
def test_missing_input_persists_failure_summary(tmp_path: Path) -> None:
    ...
    assert (output_dir / "v22_037_r2b_summary.json").exists()


def test_enrichment_time_is_used_as_disclosed_historical_asof_fallback(tmp_path: Path) -> None:
    historical_valuation = datetime(2026, 7, 5, 12, 1, 45, tzinfo=ET)
    historical_expiry = datetime(2026, 7, 6, 16, 0, 0, tzinfo=ET)
    t = (historical_expiry - historical_valuation).total_seconds() / (365.0 * 86400.0)
    price = m.bs_price("CALL", 100.0, 100.0, t, 0.043, 0.0, 0.55)
    row = {
        "option_code": "US.TEST260706C100000",
        "underlying": "TEST",
        "call_put": "CALL",
        "strike": "100",
        ...
        "ask": str(price + 0.005),
        "last": str(price),
        "underlying_price_after": "100",
        "enrichment_time_utc": "2026-07-05T16:01:45Z",
        "underlying_snapshot_age_minutes": "60",
    }
    result, checks = m.recalculate_row(row, 2, tmp_path / "input.csv", m.Config())
    assert result["valuation_time_source"] == "ENRICHMENT_TIME_UTC_FALLBACK"
    assert result["valuation_timestamp_et"].startswith("2026-07-05T12:01:45")
    assert result["days_to_expiry"] > 1.0
    assert result["synthetic_iv"] == pytest.approx(0.55, abs=1e-5)
    assert result["valuation_timestamp_trust_pass"] is False
    assert result["eligible_for_research_ranking"] is False
    assert any(
        c["check_name"] == "valuation_timestamp_trust_pass" and c["passed"] == "False"
        for c in checks
    )


def test_underlying_timestamp_is_reconstructed_from_snapshot_age(tmp_path: Path) -> None:
    historical_valuation = datetime(2026, 7, 5, 12, 1, 45, tzinfo=ET)
    historical_expiry = datetime(2026, 7, 7, 16, 0, 0, tzinfo=ET)
    t = (historical_expiry - historical_valuation).total_seconds() / (365.0 * 86400.0)
    price = m.bs_price("PUT", 100.0, 100.0, t, 0.043, 0.0, 0.45)
    row = {
        "option_code": "US.TEST260707P100000",
        "underlying": "TEST",
        "call_put": "PUT",
        "strike": "100",
        ...
        "underlying_price_after": "100",
        "enrichment_time_utc": "2026-07-05T16:01:45Z",
        "underlying_snapshot_age_minutes": "664.8537",
    }
    result, _ = m.recalculate_row(row, 2, tmp_path / "input.csv", m.Config())
    assert result["underlying_quote_time_source"] == "RECONSTRUCTED_FROM_ENRICHMENT_TIME_MINUS_SNAPSHOT_AGE"
    assert result["underlying_quote_timestamp_et"]
    assert result["quote_alignment_seconds"] == pytest.approx(664.8537 * 60, abs=0.01)
    assert result["timestamp_alignment_pass"] is False
    assert "UNDERLYING_TIME_RECONSTRUCTED_FROM_AGE" in result["quality_flags"]


def test_runtime_date_after_expiry_does_not_override_historical_enrichment_time(tmp_path: Path) -> None:
    row = {
        "option_code": "US.QQQ260706C700000",
        "underlying": "QQQ",
        "call_put": "CALL",
        "strike": "700",
        "expiration": "2026-07-06",
    ...
    assert "TIME_TO_EXPIRY_VALID" not in result["quality_flags"]
