...
REVISION = "V22.037_R2D"
OUTPUT_DIR_NAME = "V22.037_R2D_MULTI_UNDERLYING_RATE_LIMITED_SAME_SNAPSHOT_CAPTURE_AND_IV_GREEKS_VALIDATION_RESEARCH_ONLY"
PASS_STATUS = "PASS_V22_037_R2D_SAME_SNAPSHOT_CAPTURE_AND_IV_GREEKS_VALIDATED"
WARN_STATUS = "WARN_V22_037_R2D_CAPTURED_WITH_ALIGNMENT_OR_QUALITY_WARNINGS"
WARN_PARTIAL_STATUS = "WARN_V22_037_R2D_PARTIAL_UNDERLYING_CAPTURE_WITH_QUALITY_WARNINGS"
FAIL_CAPTURE = "FAIL_V22_037_R2D_NO_VALID_SAME_SNAPSHOT_OPTION_ROWS"
FAIL_CHILD = "FAIL_V22_037_R2D_IV_GREEKS_CHILD_FAILED"
FAIL_EXCEPTION = "FAIL_V22_037_R2D_UNHANDLED_EXCEPTION"
...
WARN_PARTIAL_DECISION = "PARTIAL_MULTI_UNDERLYING_IV_GREEKS_AVAILABLE_WITH_FILTER_REQUIRED_RESEARCH_ONLY"
...
CHAIN_AUDIT_FIELDS = [
    "underlying", "underlying_code", "request_range_start", "request_range_end", "request_attempts",
    "rate_limit_retry_count", "request_start_utc", "request_end_utc", "expiration",
    "expiration_distance_days", "chain_row_count", "selected_row_count", "status", "error_message",
]

UNDERLYING_AUDIT_FIELDS = [
    "capture_id", "underlying", "status", "error_message", "expiration_count",
    "option_chain_request_count", "rate_limit_retry_count", "option_chain_static_row_count",
    "selected_contract_count", "snapshot_batch_count", "captured_option_row_count",
    "timestamp_alignment_pass_count",
]

SUMMARY_FIELDS = [
    "revision", "final_status", "final_decision", "repo_root", "output_root", "output_dir", "run_id",
    "run_start_utc", "run_end_utc", "underlyings_requested", "underlying_count",
    "underlying_success_count", "underlying_failure_count", "expiration_count",
    "option_chain_request_count", "rate_limit_retry_count", "option_chain_static_row_count",
    "selected_contract_count", "snapshot_batch_count", "captured_option_row_count",
    "same_snapshot_request_count", "explicit_option_quote_timestamp_count",
    ...
    "underlying_audit_path", "iv_child_executed", "iv_child_exit_code", "iv_child_summary_path",
    "iv_child_final_status", "iv_child_final_decision", "synthetic_iv_solved_count",
    "synthetic_iv_coverage_ratio", "greeks_valid_count", "research_ranking_eligible_count",
    "research_ranking_eligible_ratio", "quality_tier_a_count", "quality_tier_b_count",
    "quality_tier_c_count", "quality_rejected_count", "research_only", "official_adoption_allowed",
    "broker_action_allowed", "error_message",
]
...
class Config:
    ...
    option_chain_rate_limit_calls: int = 9
    option_chain_rate_limit_window_seconds: float = 30.0
    option_chain_max_attempts: int = 2
    option_chain_retry_seconds: float = 31.0
...
def is_rate_limit_error(value: Any) -> bool:
    ...
    markers = (
        "frequency limit", "frequency restriction", "too frequent", "rate limit",
        "请求太频繁", "频率限制", "每 30 秒", "每30秒", "30 seconds",
    )
...
class SlidingWindowRateLimiter:
    ...
    def acquire(self) -> float:
        ...
        if len(self.calls) >= self.max_calls:
            waited = max(0.0, self.window_seconds - (now - self.calls[0]) + 0.05)
            if waited > 0:
                self.sleep_fn(waited)
                self.wait_count += 1
...
def get_option_chain_range_with_retry(
    ctx: Any,
    code: str,
    start_date: date,
    end_date: date,
    ...
    *,
...
) -> tuple[Any, Any, int, int, datetime, datetime]:
    attempts = 0
    retries = 0
    first_start: datetime | None = None
    last_end: datetime | None = None
    last_ret: Any = -1
    last_data: Any = "No option-chain request executed."
    max_attempts = max(1, int(config.option_chain_max_attempts))
    for attempt in range(1, max_attempts + 1):
        ...
        last_ret, last_data = ctx.get_option_chain(
            code, start=start_date.isoformat(), end=end_date.isoformat()
        )
        last_end = now_fn()
        if ret_ok(last_ret, module_ret_ok):
            return last_ret, last_data, attempts, retries, first_start, last_end
        if attempt < max_attempts and is_rate_limit_error(last_data):
            retries += 1
            sleep_fn(max(0.0, float(config.option_chain_retry_seconds)))
            continue
        break
...
def collect_static_chain(
    ...
    *,
    ...
    if not expirations:
        raise RuntimeError(
            f"No unexpired option expirations within DTE {config.min_dte}..{config.max_dte} for {code}"
        )

    # Official API permits a date range of at most 30 days. The configured default
    # window is 14 DTE, so one range request per underlying replaces the old
    # one-request-per-expiration pattern and stays safely below 10 requests/30s.
    range_start = expirations[0][0]
    range_end = expirations[-1][0]
    if (range_end - range_start).days > 30:
        raise ValueError(
            f"Option-chain request range exceeds 30 days for {code}: {range_start}..{range_end}"
        )
    ret, chain_data, attempts, retries, request_start, request_end = get_option_chain_range_with_retry(
        ctx, code, range_start, range_end, config, module_ret_ok, limiter,
        sleep_fn=sleep_fn, now_fn=now_fn,
    )
    if not ret_ok(ret, module_ret_ok):
        error = str(chain_data)
        audit = [
            {
                "underlying": symbol,
                "underlying_code": code,
                "request_range_start": range_start.isoformat(),
                "request_range_end": range_end.isoformat(),
                "request_attempts": attempts,
                "rate_limit_retry_count": retries,
                "request_start_utc": request_start.astimezone(UTC).isoformat(),
                "request_end_utc": request_end.astimezone(UTC).isoformat(),
                "expiration": expiry.isoformat(),
                "expiration_distance_days": distance,
                "chain_row_count": 0,
        ...
        return [], audit, len(expirations), 1, retries

    raw_chain = records_from_table(chain_data)
    target_expirations = {expiry.isoformat(): distance for expiry, distance in expirations}
    normalized_all = [
        item for item in (normalize_chain_row(row, symbol) for row in raw_chain) if item
    ]
    normalized = [
        row for row in normalized_all if str(row.get("expiration", "")) in target_expirations
    ]
    counts: dict[str, int] = {}
    for row in normalized:
        expiry_text = str(row.get("expiration", ""))
        counts[expiry_text] = counts.get(expiry_text, 0) + 1
    audit: list[dict[str, Any]] = []
    for expiry, distance in expirations:
        expiry_text = expiry.isoformat()
        count = counts.get(expiry_text, 0)
        audit.append({
            "underlying": symbol,
            "underlying_code": code,
            "request_range_start": range_start.isoformat(),
            ...
            "expiration_distance_days": distance,
            "chain_row_count": count,
            "selected_row_count": count,
            "status": "PASS" if count > 0 else "WARN",
            "error_message": "" if count > 0 else "No normalized contracts returned for selected expiration.",
        })
    return normalized, audit, len(expirations), 1, retries
...
def capture_symbol(
    ...
    code = underlying_code(symbol)
    static_rows, chain_audit, expiration_count, chain_request_count, retry_count = collect_static_chain(
        ctx, symbol, as_of_et, config, module_ret_ok, limiter,
        sleep_fn=sleep_fn, now_fn=now_fn,
    )
    ...
    return (
        captured, batch_audit, chain_audit, len(static_rows), expiration_count,
        chain_request_count, retry_count,
    )
...
def final_status_for(
    ...
    *,
    underlying_failure_count: int = 0,
    regular_session_pass: bool = False,
) -> tuple[str, str]:
    if captured_count <= 0:
        return FAIL_CAPTURE, FAIL_DECISION
    if child_executed and (
        child_exit_code != 0 or str(child_summary.get("final_status", "")).startswith("FAIL_")
    ):
        return FAIL_CHILD, FAIL_DECISION
    if underlying_failure_count > 0:
        return WARN_PARTIAL_STATUS, WARN_PARTIAL_DECISION
    ...
    if child_executed:
        ...
        if (
            solved > 0
            and eligible > 0
            and aligned_ratio >= config.min_alignment_pass_ratio
            and regular_session_pass
        ):
...
def make_summary(
    ...
    underlying_audit: Sequence[Mapping[str, Any]],
    expiration_count: int,
    option_chain_request_count: int,
    ...
    aligned = sum(
        str(row.get("timestamp_alignment_pass")).lower() in {"true", "1"}
        or row.get("timestamp_alignment_pass") is True
        for row in captured
    )
    same_request = sum(
        str(row.get("same_snapshot_request")).lower() in {"true", "1"}
        or row.get("same_snapshot_request") is True
        for row in captured
    )
    ...
    success_count = sum(str(row.get("status", "")).upper() == "PASS" for row in underlying_audit)
    failure_count = sum(str(row.get("status", "")).upper() == "FAIL" for row in underlying_audit)
    status, decision = final_status_for(
        captured_count,
        aligned,
        config,
        child_executed,
        child_exit_code,
        child_summary,
        underlying_failure_count=failure_count,
        regular_session_pass=regular_session_pass,
    )
    if error_message and captured_count == 0:
    ...
    return {
        ...
        "underlying_success_count": success_count,
        "underlying_failure_count": failure_count,
        "expiration_count": expiration_count,
        "option_chain_request_count": option_chain_request_count,
        ...
        "underlying_audit_path": str(underlying_audit_path),
...
def render_report(summary: Mapping[str, Any]) -> str:
    lines = [
        "V22.037 R2D Multi-Underlying Rate-Limited Same-Snapshot Capture and IV/Greeks Validation",
        "=" * 82,
        ...
        f"Underlying success/failure: {summary.get('underlying_success_count', 0)}/{summary.get('underlying_failure_count', 0)}",
        f"Option-chain requests: {summary.get('option_chain_request_count', 0)}",
        f"Rate-limit retries: {summary.get('rate_limit_retry_count', 0)}",
...
def execute(
    ...
    run_id = f"v22_037_r2d_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S_%f')}_{uuid.uuid4().hex[:8]}"
    ...
    underlying_audit_path = output_dir / "underlying_capture_audit.csv"
    summary_path = output_dir / "v22_037_r2d_summary.json"
    report_path = output_dir / "V22.037_R2D_multi_underlying_rate_limited_same_snapshot_capture_and_iv_greeks_validation_report.txt"
    ...
    underlying_audit_all: list[dict[str, Any]] = []
    static_count = 0
    selected_count = 0
    expiration_count = 0
    option_chain_request_count = 0
    ...
    active_limiter = limiter or SlidingWindowRateLimiter(
        config.option_chain_rate_limit_calls,
        config.option_chain_rate_limit_window_seconds,
        sleep_fn=sleep_fn,
    )
    try:
        ...
        try:
            for symbol in underlyings:
                try:
                    ...
                    selected_for_symbol = sum(
                        int(row.get("requested_option_count", 0) or 0) for row in batches
                    )
                    selected_count += selected_for_symbol
                    expiration_count += exp_count
                    option_chain_request_count += chain_requests
                    rate_limit_retry_count += retry_count
                    aligned_for_symbol = sum(
                        row.get("timestamp_alignment_pass") is True
                        or str(row.get("timestamp_alignment_pass", "")).lower() in {"true", "1"}
                        for row in rows
                    )
                    status = "PASS" if rows else "FAIL"
                    chain_errors = sorted({
                        str(item.get("error_message", "")).strip()
                        for item in chain_audit
                        if str(item.get("error_message", "")).strip()
                    })
                    failure_reason = "" if rows else (
                        " | ".join(chain_errors) or "No valid same-snapshot option rows captured."
                    )
                    underlying_audit_all.append({
                        "capture_id": capture_id,
                        "underlying": symbol,
                        "status": status,
                        ...
                        "option_chain_request_count": chain_requests,
                        "rate_limit_retry_count": retry_count,
                        "option_chain_static_row_count": static_rows,
                        "selected_contract_count": selected_for_symbol,
                        "snapshot_batch_count": len(batches),
                        "captured_option_row_count": len(rows),
                        "timestamp_alignment_pass_count": aligned_for_symbol,
                    })
                    ...
                    underlying_audit_all.append({
                        ...
                        "option_chain_request_count": 0,
                        "rate_limit_retry_count": 0,
                        "option_chain_static_row_count": 0,
                        "selected_contract_count": 0,
                        "snapshot_batch_count": 0,
                        "captured_option_row_count": 0,
                        "timestamp_alignment_pass_count": 0,
                    })
                    continue
        finally:
        ...
        write_csv(underlying_audit_path, underlying_audit_all, UNDERLYING_AUDIT_FIELDS)
        ...
        if not chain_audit_path.exists():
            write_csv(chain_audit_path, chain_audit_all, CHAIN_AUDIT_FIELDS)
        if not underlying_audit_path.exists():
            write_csv(underlying_audit_path, underlying_audit_all, UNDERLYING_AUDIT_FIELDS)
    ended = utc_now_iso()
    summary = make_summary(
        ...
        captured_all,
        capture_input,
        underlying_audit_path,
        child_executed,
        child_exit_code,
        child_summary,
        child_summary_path,
        regular_session_clock_pass(current_et),
        config,
        error_message,
    )
    write_json(summary_path, summary)
    report_path.write_text(render_report(summary), encoding="utf-8")
    write_json(latest_pointer_path, {
        "revision": REVISION,
        "run_id": run_id,
        "output_dir": str(output_dir),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="V22.037 R2D research-only multi-underlying rate-limited same-snapshot capture and IV/Greeks validation")
    parser.add_argument("--repo-root", type=Path, default=default_repo_root())
    parser.add_argument("--output-dir", type=Path, default=None, help="Output root; each run is written to a unique runs/<run_id> directory.")
    ...
    parser.add_argument("--option-chain-rate-limit-calls", type=int, default=Config.option_chain_rate_limit_calls)
    parser.add_argument("--option-chain-rate-limit-window-seconds", type=float, default=Config.option_chain_rate_limit_window_seconds)
    parser.add_argument("--option-chain-max-attempts", type=int, default=Config.option_chain_max_attempts)
    parser.add_argument("--option-chain-retry-seconds", type=float, default=Config.option_chain_retry_seconds)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = Config(
        ...
        option_chain_rate_limit_calls=args.option_chain_rate_limit_calls,
        option_chain_rate_limit_window_seconds=args.option_chain_rate_limit_window_seconds,
        option_chain_max_attempts=args.option_chain_max_attempts,
        option_chain_retry_seconds=args.option_chain_retry_seconds,
    )
    try:
        ...
        run_id = f"v22_037_r2d_failed_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S_%f')}"
        failure_dir = output_root / "runs" / run_id
        ...
        write_json(failure_dir / "v22_037_r2d_summary.json", summary)
        write_json(output_root / "latest_run.json", {
            "revision": REVISION,
            "run_id": run_id,
            "output_dir": str(failure_dir),
            "summary_path": str(failure_dir / "v22_037_r2d_summary.json"),
            "final_status": FAIL_EXCEPTION,
        })
