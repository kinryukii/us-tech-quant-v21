...
REPO=Path(__file__).resolve().parents[2]; OUT_NAME='V22.076A_FAST3_0945_CONFIRMATION_PATH_RESEARCH_R1'
DATA_ROOT=Path(r'D:/us-tech-quant-data/fast3/moomoo_24h_1m'); CONTRACT=REPO/'.local_results/v22/V22.073A_FAST3_PIT_FEATURE_EXPANSION_RESEARCH_R1/feature_contract.json'
...
def run(results_root=REPO/'.local_results'):
 ...
 out=Path(results_root)/'v22'/OUT_NAME;out.parent.mkdir(parents=True,exist_ok=True);stage=Path(tempfile.mkdtemp(prefix='.076a_',dir=out.parent))
 try:
