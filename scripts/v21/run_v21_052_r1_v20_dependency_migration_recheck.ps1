...
$StageScript = Join-Path $ScriptDir "v21_052_r1_v20_dependency_migration_recheck.py"

Write-Host "STAGE_NAME=V21.052-R1_V20_DEPENDENCY_MIGRATION_RECHECK"
...
try {
    python $StageScript
    if ($LASTEXITCODE -ne 0) { throw "V21.052-R1 failed with exit code $LASTEXITCODE" }
}
