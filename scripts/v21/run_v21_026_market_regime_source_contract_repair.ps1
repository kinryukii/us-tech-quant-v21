...
$Runner = Join-Path $ScriptDir "v21_026_market_regime_source_contract_repair.py"
$Root = Split-Path -Parent (Split-Path -Parent $ScriptDir)
$Summary = Join-Path $Root "outputs\v21\factor_backtest\V21_026_MARKET_REGIME_SOURCE_CONTRACT_REPAIR_SUMMARY.csv"

python $Runner | Out-Host

if (-not (Test-Path $Summary)) {
    throw "V21.026 summary output not found: $Summary"
}

$SummaryRow = Import-Csv $Summary | Select-Object -First 1
Write-Output "STAGE_NAME=V21_026_MARKET_REGIME_SOURCE_CONTRACT_REPAIR"
Write-Output "final_status=$($SummaryRow.final_status)"
Write-Output "source_contract_repair_decision=$($SummaryRow.source_contract_repair_decision)"
