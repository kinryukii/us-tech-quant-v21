...
$Runner = Join-Path $ScriptDir "v21_024_market_regime_context_logic_repair.py"
$Root = Split-Path -Parent (Split-Path -Parent $ScriptDir)
$Summary = Join-Path $Root "outputs\v21\factor_backtest\V21_024_MARKET_REGIME_CONTEXT_LOGIC_REPAIR_SUMMARY.csv"

python $Runner | Out-Host

if (-not (Test-Path $Summary)) {
    throw "V21.024 summary output not found: $Summary"
}

$SummaryRow = Import-Csv $Summary | Select-Object -First 1
Write-Output "STAGE_NAME=V21_024_MARKET_REGIME_CONTEXT_LOGIC_REPAIR"
Write-Output "final_status=$($SummaryRow.final_status)"
Write-Output "context_logic_repair_decision=$($SummaryRow.context_logic_repair_decision)"
