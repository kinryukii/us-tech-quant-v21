...
$Runner = Join-Path $ScriptDir "v21_030_shadow_observation_ledger_maturity_tracker.py"
$Root = Split-Path -Parent (Split-Path -Parent $ScriptDir)
$Summary = Join-Path $Root "outputs\v21\shadow_observation\V21_030_SHADOW_OBSERVATION_LEDGER_MATURITY_TRACKER_SUMMARY.csv"

python $Runner | Out-Host

if (-not (Test-Path $Summary)) {
    throw "V21.030 summary output not found: $Summary"
}

$SummaryRow = Import-Csv $Summary | Select-Object -First 1
Write-Output "STAGE_NAME=V21_030_SHADOW_OBSERVATION_LEDGER_MATURITY_TRACKER"
Write-Output "final_status=$($SummaryRow.final_status)"
Write-Output "maturity_tracker_decision=$($SummaryRow.maturity_tracker_decision)"
