#!/usr/bin/env python
"""Contract test for V21.064 D continuation or price-refresh checkpoint."""
...
SCRIPT = ROOT / (
    "scripts/v21/v21_064_d_daily_maturity_continuation_or_price_refresh_check.py"
)
WRAPPER = ROOT / (
    "scripts/v21/run_v21_064_d_daily_maturity_continuation_or_price_refresh_check.ps1"
)
spec = importlib.util.spec_from_file_location("v21_064_d", SCRIPT)
...
def test_repository_wrapper() -> None:
    ...
    protected = module.protected_paths(ROOT, v63_summary.parent)
    ...
    assert validation["source_matured_count_consistent"] is True
    ...
    assert summary["continuation_state"] in module.APPROVED_STATES
    assert all(
        row["continuation_state"] == summary["continuation_state"]
        for row in details
    )
    assert all(row["research_only"] == "True" for row in details)
    for field in (
    ...
    expected_status, expected_decision, _ = module.STATUS_DECISIONS[
        summary["continuation_state"]
    ]
    assert summary["final_status"] == expected_status
    assert summary["decision"] == expected_decision
    state = summary["continuation_state"]
    assert (summary["price_refresh_recommended"] == "True") == (
        state == module.REFRESH_CHECK
    )
    assert (summary["price_gap_detected"] == "True") == (
        state == module.PRICE_GAP
    )
    assert (summary["maturity_compute_ready"] == "True") == (
        state == module.COMPUTE_READY
    )
    assert before == {path: sha(ROOT / path) for path in before}
    assert {
        path.parent.resolve()
        for path in out.glob("V21_064_*")
        if path.is_file()
    } == {out.resolve()}
...
if __name__ == "__main__":
    ...
    print(
        "PASS "
        "test_v21_064_d_daily_maturity_continuation_or_price_refresh_check"
    )
