...
$StageScript = Join-Path $ScriptDir "v21_053_r1_shared_dependency_migration_dry_run.py"

Write-Host "STAGE_NAME=V21.053-R1_SHARED_DEPENDENCY_MIGRATION_DRY_RUN"
Write-Host "RESEARCH_ONLY=TRUE"
Write-Host "MIGRATION_DRY_RUN_ONLY=TRUE"
...
try {
    python $StageScript
    if ($LASTEXITCODE -ne 0) { throw "V21.053-R1 failed with exit code $LASTEXITCODE" }
}
