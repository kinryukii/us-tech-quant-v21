...
$stateRoot = Join-Path $RepoRoot "state\fast3\agent\event_factor_cohort_r3"
$lockPath = Join-Path $stateRoot "active.lock.json"
$base = Join-Path $ExternalResultsRoot "fast3\agent_runs\event_factor_cohort_r3"
...
if (Test-Path -LiteralPath $lockPath) {
    ...
    if (Test-Path -LiteralPath $localRun -PathType Container) {
        Write-Host ""
        Write-Host "--- current R3 local artifacts ---"
        Get-ChildItem -LiteralPath $localRun -File |
            Sort-Object LastWriteTime | Select-Object Name,Length,LastWriteTime
...
Write-Host "LATEST_COMPLETED_OR_SYNCED_R3_RUN=$($latest.FullName)"
foreach ($item in @(
    @{Title="launcher_summary.json"; Path=(Join-Path $latest.FullName "launcher_summary.json")},
    @{Title="fast3_event_factor_r3_summary.json"; Path=(Join-Path $latest.FullName "fast3_event_factor_r3_summary.json")},
    @{Title="FAST3_FACTOR_LAW_FREEZE.json"; Path=(Join-Path $latest.FullName "FAST3_FACTOR_LAW_FREEZE.json")},
    @{Title="FAST3_MODEL_FREEZE.json"; Path=(Join-Path $latest.FullName "FAST3_MODEL_FREEZE.json")},
    @{Title="FAST3_EXECUTION_SANITY_AUDIT.json"; Path=(Join-Path $latest.FullName "FAST3_EXECUTION_SANITY_AUDIT.json")},
