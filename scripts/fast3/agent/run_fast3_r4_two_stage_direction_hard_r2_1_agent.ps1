...
$LimitsPath = Join-Path $RepoRoot "config\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R2_1_1_LIMITS.json"
$PromptPath = Join-Path $RepoRoot "docs\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R2_1_1_AGENT.md"
...
$LockPath = Join-Path $StateRoot "active_hard_r2_1.lock.json"
...
try {
    ...
    if ($preexistingAgentTargets.Count -gt 0) {
        throw "R4 hard-R2.1 implementation target already exists. It was preserved and not overwritten. Use a clean new target version or inspect the existing implementation."
    }
    ...
    $instruction = @"
Implement and unit-test FAST3 R4 hard-storage R2.1.
...
'@
    ...
    $testFile = Join-Path $RepoRoot "fast3\tests\unit\test_fast3_r4_two_stage_direction_hard_r2_1_1.py"
...
'@
    ...
    $researchScript = Join-Path $RepoRoot "fast3\scripts\run\fast3_r4_two_stage_direction_hard_r2_1_1.py"
