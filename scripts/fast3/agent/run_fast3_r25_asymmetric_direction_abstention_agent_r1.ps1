...
$AgentRuntimeRoot = Join-Path $ResultsRoot "runtime\fast3\r25_agent_launcher\$RunId"
$AgentScratchRoot = Join-Path $ResultsRoot "scratch\fast3\r25_agent_launcher\$RunId"
$AgentFrozenRoot = Join-Path $ResultsRoot "frozen\fast3\r25_agent_launcher\$RunId"
$AgentArchiveRoot = Join-Path $ResultsRoot "archive\fast3\r25_agent_launcher\$RunId"
$AgentCacheRoot = Join-Path $CacheRoot "fast3\r25_agent_launcher\$RunId"
$TaskTemplate = Join-Path $PackageRoot "config\FAST3_R25_ASYMMETRIC_DIRECTION_ABSTENTION_TASK_R1.md"
...
$LauncherSummaryPath = Join-Path $AgentFrozenRoot "R25_AGENT_LAUNCHER_SUMMARY.json"
$LauncherReportPath = Join-Path $AgentFrozenRoot "R25_AGENT_LAUNCHER_REPORT.md"
...
$summary = [ordered]@{
    ...
    FINAL_DECISION = "STOP_R25_AGENT_NOT_COMPLETED"
...
try {
    Write-Section "FAST3 R25 agent preflight"
...
exit $LASTEXITCODE
'@
        [System.IO.File]::WriteAllText($GitGuardPath, $gitGuard, [System.Text.UTF8Encoding]::new($false))
        $gitCmd = '@echo off' + "`r`n" +
            'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git_guard.ps1" %*' + "`r`n" +
            'exit /b %ERRORLEVEL%' + "`r`n"
        [System.IO.File]::WriteAllText($GitCmdPath, $gitCmd, [System.Text.ASCIIEncoding]::new())
    ...
    [System.IO.File]::WriteAllText($CanonicalBeforePath, $CanonicalBefore, [System.Text.ASCIIEncoding]::new())
    ...
    $env:TEMP = $env:TMP
    New-Item -ItemType Directory -Force -Path $env:TMP, $env:PYTHONPYCACHEPREFIX | Out-Null
    ...
    Write-Section "Launch local Codex R25 agent"
    ...
    [System.IO.File]::WriteAllText($CodexCmdPath, $cmdText, [System.Text.ASCIIEncoding]::new())
    ...
    if (Test-Path -LiteralPath $FinalMessagePath -PathType Leaf) {
        ...
        $decisionMatch = [regex]::Match($finalText, "(?im)^\s*FINAL_DECISION\s*=\s*(.+?)\s*$")
        if ($decisionMatch.Success) {
            $summary.FINAL_DECISION = $decisionMatch.Groups[1].Value.Trim()
        } elseif ($summary.CODEX_EXIT_CODE -eq 0) {
            $summary.FINAL_DECISION = "CODEX_COMPLETED_REVIEW_FINAL_MESSAGE"
        } else {
            $summary.FINAL_DECISION = "STOP_R25_CODEX_EXIT_$($summary.CODEX_EXIT_CODE)"
        }
    } elseif ($summary.CODEX_EXIT_CODE -eq 0) {
        $summary.FINAL_DECISION = "STOP_R25_FINAL_MESSAGE_MISSING"
    } else {
        $summary.FINAL_DECISION = "STOP_R25_CODEX_EXIT_$($summary.CODEX_EXIT_CODE)"
    }
    ...
    $summary.FINAL_DECISION = "STOP_R25_AGENT_LAUNCHER_FAILED:$($_.Exception.Message)"
}
finally {
    Write-Section "FAST3 R25 agent post-run audit"

    try {
        $CanonicalAfter = Get-TreeMetadataHash -Root $CanonicalRoot
        [System.IO.File]::WriteAllText($CanonicalAfterPath, $CanonicalAfter, [System.Text.ASCIIEncoding]::new())
        $summary.CANONICAL_METADATA_UNCHANGED = ($CanonicalBefore -eq $CanonicalAfter)
        if (-not $summary.CANONICAL_METADATA_UNCHANGED) {
            $summary.FINAL_DECISION = "STOP_R25_CANONICAL_METADATA_CHANGED"
        }
    }
    catch {
        $summary.CANONICAL_METADATA_UNCHANGED = $false
        $summary.FINAL_DECISION = "STOP_R25_CANONICAL_POST_SNAPSHOT_FAILED:$($_.Exception.Message)"
    }

    try {
        $LocalResultsAfter = Get-LocalResultsInventory -RepoRoot $RepoRoot
        $beforeSet = @{}
        foreach ($item in $LocalResultsBefore) { $beforeSet[[string]$item] = $true }
        $newLocal = @($LocalResultsAfter | Where-Object { -not $beforeSet.ContainsKey([string]$_) })
        $summary.NEW_LOCAL_RESULTS_WRITE_COUNT = $newLocal.Count
        if ($newLocal.Count -gt 0) {
            $summary.FINAL_DECISION = "STOP_R25_NEW_LOCAL_RESULTS_WRITES"
        }
    }
    catch {
        $summary.NEW_LOCAL_RESULTS_WRITE_COUNT = -1
        $summary.FINAL_DECISION = "STOP_R25_LOCAL_RESULTS_AUDIT_FAILED:$($_.Exception.Message)"
    }

    try {
        $summary.UNTRACKED_FILE_PRESERVATION_PASS = Test-UntrackedPathsPreserved `
            -Before $UntrackedBefore -RepoRoot $RepoRoot
        if (-not $summary.UNTRACKED_FILE_PRESERVATION_PASS) {
            $summary.FINAL_DECISION = "STOP_R25_PREEXISTING_UNTRACKED_PATH_MISSING"
        }
    }
    catch {
        $summary.UNTRACKED_FILE_PRESERVATION_PASS = $false
        $summary.FINAL_DECISION = "STOP_R25_UNTRACKED_PRESERVATION_AUDIT_FAILED:$($_.Exception.Message)"
    }
    ...
    $report = @"
# FAST3 R25 Agent Launcher Report
...
The authoritative R25 research decision, if produced, is in the R25 frozen run root
created by the implemented hard-storage launcher. This outer report only audits the
deployment agent process.
"@
    [System.IO.File]::WriteAllText($LauncherReportPath, $report, [System.Text.UTF8Encoding]::new($false))

    foreach ($path in @($TaskPath, $EventsPath, $StderrPath, $FinalMessagePath, $CodexCmdPath)) {
        if (Test-Path -LiteralPath $path -PathType Leaf) {
            Copy-Item -LiteralPath $path -Destination $AgentArchiveRoot -Force
        }
    }

    Write-Section "FAST3 R25 agent launcher summary"
...
if ([string]$summary.FINAL_DECISION -like "STOP_*") { exit 3 }
exit 0
