...
$LimitsPath = Join-Path $RepoRoot "config\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R22_LIMITS.json"
$PromptPath = Join-Path $RepoRoot "docs\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R22_AGENT.md"
...
$expectedControlRefs = @(
    $LimitsPath,
    $PromptPath,
    (Join-Path $RepoRoot "scripts\fast3\agent\run_fast3_r4_two_stage_direction_hard_r22_agent.ps1"),
    (Join-Path $RepoRoot "scripts\fast3\agent\show_fast3_r4_two_stage_direction_hard_r22_status.ps1")
)
foreach ($controlRef in $expectedControlRefs) {
    if (-not (Test-Path -LiteralPath $controlRef -PathType Leaf)) {
        throw "R22_CONTROL_REFERENCE_SELFTEST_FAILED: $controlRef"
    }
}
Write-Host "R22_CONTROL_REFERENCE_SELFTEST_PASS=true"
...
$LockPath = Join-Path $StateRoot "active_hard_r22.lock.json"
...
"@ | Set-Content -LiteralPath $gitBlocker -Encoding ASCII
    ...
    $instruction = @"
Implement and unit-test FAST3 R4 hard-storage R2.2.
...
'@
    ...
    $testFile = Join-Path $RepoRoot "fast3\tests\unit\test_fast3_r4_two_stage_direction_hard_r22.py"
...
'@
    ...
    $researchScript = Join-Path $RepoRoot "fast3\scripts\run\fast3_r4_two_stage_direction_hard_r22.py"
...
'@
    ...
    try {
        ...
        $RepoViolations.Add("POSTRUN_SNAPSHOT_AUDIT_FAILED:" + $_.Exception.Message)
    }

    if ($RepoSnapshotViolations.Count -gt 0 -or
        $RepoOpaqueSnapshotViolations.Count -gt 0 -or
        $CanonicalSnapshotViolations.Count -gt 0 -or
        $RepoViolations.Count -gt 0 -or
        $DataEvents.Count -gt 0 -or
        $LocalEvents.Count -gt 0 -or
    ...
    $storagePass = (
        $AclGuardActive -and
        $RepoSnapshotViolations.Count -eq 0 -and
        $RepoOpaqueSnapshotViolations.Count -eq 0 -and
        $CanonicalSnapshotViolations.Count -eq 0 -and
        $RepoViolations.Count -eq 0 -and
        $DataEvents.Count -eq 0 -and
        $LocalEvents.Count -eq 0 -and
    ...
    $audit = [ordered]@{
        ...
        canonical_watch_event_count=$DataEvents.Count
        canonical_watch_events=@($DataEvents)
        canonical_snapshot_violation_count=$CanonicalSnapshotViolations.Count
        canonical_snapshot_violations=$CanonicalSnapshotViolations
        repo_watch_violation_count=$RepoViolations.Count
        repo_watch_violations=@($RepoViolations)
        repo_snapshot_violation_count=$RepoSnapshotViolations.Count
        repo_snapshot_violations=$RepoSnapshotViolations
        repo_opaque_snapshot_violation_count=$RepoOpaqueSnapshotViolations.Count
        ...
        new_local_results_write_count=$LocalEvents.Count
        local_results_events=@($LocalEvents)
        git_mutation_performed=$false
        forbidden_git_commands_executed=$false
        preexisting_untracked_files_preserved=($RepoSnapshotViolations.Count -eq 0 -and $LocalEvents.Count -eq 0)
        canonical_write_count=if($CanonicalSnapshotViolations.Count -eq 0 -and $DataEvents.Count -eq 0){0}else{$CanonicalSnapshotViolations.Count + $DataEvents.Count}
        repo_result_write_count=if($RepoSnapshotViolations.Count -eq 0 -and $RepoViolations.Count -eq 0){0}else{$RepoSnapshotViolations.Count + $RepoViolations.Count}
    ...
    Write-Host "CANONICAL_WATCH_EVENT_COUNT=$($DataEvents.Count)"
    Write-Host "REPO_WATCH_VIOLATION_COUNT=$($RepoViolations.Count)"
    Write-Host "NEW_LOCAL_RESULTS_WRITE_COUNT=$($LocalEvents.Count)"
    Write-Host "REPO_SNAPSHOT_VIOLATION_COUNT=$($RepoSnapshotViolations.Count)"
    Write-Host "REPO_OPAQUE_SNAPSHOT_VIOLATION_COUNT=$($RepoOpaqueSnapshotViolations.Count)"
    Write-Host "CANONICAL_SNAPSHOT_VIOLATION_COUNT=$($CanonicalSnapshotViolations.Count)"
    Write-Host "PREEXISTING_UNTRACKED_FILES_PRESERVED=$(($RepoSnapshotViolations.Count -eq 0 -and $LocalEvents.Count -eq 0).ToString().ToLowerInvariant())"
