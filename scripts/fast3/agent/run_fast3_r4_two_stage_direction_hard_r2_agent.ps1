...
function Snapshot-Repo([string]$Root,[string[]]$SkipPrefixes) {
    $map = [ordered]@{}
    foreach ($f in Get-ChildItem -LiteralPath $Root -Force -File -Recurse -ErrorAction Stop) {
        $fullPath = [IO.Path]::GetFullPath($f.FullName)
        $skip = $false
        foreach ($prefix in $SkipPrefixes) {
            $p = (Full $prefix)
            if ($fullPath -eq $p -or $fullPath.StartsWith($p + '\',[StringComparison]::OrdinalIgnoreCase)) {
                $skip = $true
                break
            }
        }
...
$LimitsPath = Join-Path $RepoRoot "config\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R2_LIMITS.json"
$PromptPath = Join-Path $RepoRoot "docs\fast3\agent\FAST3_R4_TWO_STAGE_DIRECTION_HARD_R2_AGENT.md"
...
$LockPath = Join-Path $StateRoot "active_hard_r2.lock.json"
...
try {
    ...
    if ($preexistingAgentTargets.Count -gt 0) {
        throw "R4 hard-R2 implementation target already exists. It was preserved and not overwritten. Use a clean new target version or inspect the existing implementation."
    }

    $skipPrefixes = @((Join-Path $RepoRoot ".git"),(Join-Path $RepoRoot ".local_results"))
    $BeforeRepo = Snapshot-Repo $RepoRoot $skipPrefixes
    ...
    $instruction = @"
Implement and unit-test FAST3 R4 hard-storage R2.
...
'@
    ...
    $testFile = Join-Path $RepoRoot "fast3\tests\unit\test_fast3_r4_two_stage_direction_hard_r2.py"
...
'@
    ...
    $researchScript = Join-Path $RepoRoot "fast3\scripts\run\fast3_r4_two_stage_direction_hard_r2.py"
...
'@
    ...
    try {
        $afterRepo = Snapshot-Repo $RepoRoot @((Join-Path $RepoRoot ".git"),(Join-Path $RepoRoot ".local_results"))
        $relativeAllowed = @{}
        foreach ($key in @($allowedFiles.Keys)) {
