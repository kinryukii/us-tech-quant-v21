...
function Run-MonitoredProcess(
    ...
    try {
        ...
        throw "MONITORED_PROCESS_WAIT_FAILED: $($_.Exception.Message)"
    }
    ...
    if ($null -eq $capturedExitCode) {
        throw "MONITORED_PROCESS_EXIT_CODE_MISSING"
    }
    return [int]$capturedExitCode
}
...
$expectedControlRefs = @(
    $LimitsPath,
    $PromptPath,
    (Join-Path $RepoRoot "scripts\fast3\agent\resume_fast3_r4_two_stage_direction_hard_r24_existing.ps1"),
...
$LockPath = Join-Path $StateRoot "active_hard_r24_resume.lock.json"
if (Test-Path -LiteralPath $LockPath -PathType Leaf) {
    try {
        $old = Read-Json $LockPath
        if (Get-Process -Id ([int]$old.pid) -ErrorAction SilentlyContinue) {
            throw "R4 hard-storage resume is already running. PID=$($old.pid)"
        }
    } catch {
...
try {
    ...
    $implementationFreezePath = Join-Path $FrozenRoot "FAST3_R4_EXISTING_IMPLEMENTATION_FREEZE.json"
    Write-Json ([ordered]@{
        run_id=$RunId
...
"@ | Set-Content -LiteralPath $gitBlocker -Encoding ASCII
    ...
    # Phase 1 resume: do not rerun Codex. Validate the existing R24 implementation.
    ...
    foreach ($path in $requiredImplementationPaths) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            $ImplementationExit = 2
            throw "R24_EXISTING_IMPLEMENTATION_REQUIRED_FILE_MISSING: $path"
        }
    }

    $syntaxStdout = Join-Path $RuntimeRoot "implementation_syntax_stdout.log"
    $syntaxStderr = Join-Path $RuntimeRoot "implementation_syntax_stderr.log"
    $syntaxWrapper = Join-Path $RunCacheRoot "validate_existing_implementation.py"
    @'
...
'@.Replace("__REPO_ROOT__", $RepoRoot) | Set-Content -LiteralPath $syntaxWrapper -Encoding UTF8

    $ImplementationExit = Run-MonitoredProcess `
        $python `
        @('"{0}"' -f $syntaxWrapper) `
        $RepoRoot $syntaxStdout $syntaxStderr `
    ...
    if (-not [string]::IsNullOrWhiteSpace($ViolationReason)) {
        ...
        throw "Storage violation during existing-implementation validation: $ViolationReason"
    }
    if ($ImplementationExit -ne 0) {
        $FinalDecision = "STOP_R4_IMPLEMENTATION_OR_TEST_FAILED"
        throw "Existing R24 implementation syntax validation failed. Exit=$ImplementationExit"
    }
