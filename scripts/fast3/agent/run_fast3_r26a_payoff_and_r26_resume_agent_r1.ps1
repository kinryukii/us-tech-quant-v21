...
$AgentRuntimeRoot = Join-Path $ResultsRoot "runtime\fast3\r26a_r26_agent_launcher\$RunId"
$AgentScratchRoot = Join-Path $ResultsRoot "scratch\fast3\r26a_r26_agent_launcher\$RunId"
$AgentFrozenRoot = Join-Path $ResultsRoot "frozen\fast3\r26a_r26_agent_launcher\$RunId"
$AgentArchiveRoot = Join-Path $ResultsRoot "archive\fast3\r26a_r26_agent_launcher\$RunId"
$AgentCacheRoot = Join-Path $CacheRoot "fast3\r26a_r26_agent_launcher\$RunId"
$TaskTemplate = Join-Path $PackageRoot "config\FAST3_R26A_EXECUTABLE_PAYOFF_LEDGER_AND_R26_RESUME_TASK_R1.md"
...
$LauncherSummaryPath = Join-Path $AgentFrozenRoot "R26A_R26_AGENT_LAUNCHER_SUMMARY.json"
$LauncherReportPath = Join-Path $AgentFrozenRoot "R26A_R26_AGENT_LAUNCHER_REPORT.md"
...
$summary = [ordered]@{
    ...
    FINAL_DECISION = "STOP_R26A_R26_AGENT_NOT_COMPLETED"
...
try {
    Write-Section "FAST3 R26A payoff ledger / R26 resume preflight"
...
'@
    ...
    Write-Section "Launch local Codex R26A / R26 agent"
    ...
    if (Test-Path -LiteralPath $FinalMessagePath -PathType Leaf) {
        ...
        if ($null -ne $found) {
            ...
            $summary.FINAL_DECISION = "STOP_R26A_R26_CODEX_EXIT_$($summary.CODEX_EXIT_CODE)"
        }
    } elseif ($summary.CODEX_EXIT_CODE -eq 0) {
        $summary.FINAL_DECISION = "STOP_R26A_R26_FINAL_MESSAGE_MISSING"
    } else {
        $summary.FINAL_DECISION = "STOP_R26A_R26_CODEX_EXIT_$($summary.CODEX_EXIT_CODE)"
    }
    ...
    $summary.FINAL_DECISION = "STOP_R26A_R26_AGENT_LAUNCHER_FAILED:$($_.Exception.Message)"
}
finally {
    Write-Section "FAST3 R26A / R26 post-run audit"
    ...
    try {
        ...
        if (-not $summary.CANONICAL_METADATA_UNCHANGED) {
            $summary.FINAL_DECISION = "STOP_R26A_R26_CANONICAL_METADATA_CHANGED"
        }
    }
    catch {
        $summary.CANONICAL_METADATA_UNCHANGED = $false
        $summary.FINAL_DECISION =
            "STOP_R26A_R26_CANONICAL_POST_SNAPSHOT_FAILED:$($_.Exception.Message)"
    }
    ...
    try {
        ...
        if ($newLocal.Count -gt 0) {
            $summary.FINAL_DECISION = "STOP_R26A_R26_NEW_LOCAL_RESULTS_WRITES"
        }
    }
    catch {
        $summary.NEW_LOCAL_RESULTS_WRITE_COUNT = -1
        $summary.FINAL_DECISION =
            "STOP_R26A_R26_LOCAL_RESULTS_AUDIT_FAILED:$($_.Exception.Message)"
    }
    ...
    try {
        ...
        if (-not $summary.UNTRACKED_FILE_PRESERVATION_PASS) {
            $summary.FINAL_DECISION =
                "STOP_R26A_R26_PREEXISTING_UNTRACKED_PATH_MISSING"
        }
    }
    catch {
        $summary.UNTRACKED_FILE_PRESERVATION_PASS = $false
        $summary.FINAL_DECISION =
            "STOP_R26A_R26_UNTRACKED_PRESERVATION_AUDIT_FAILED:$($_.Exception.Message)"
    }
    ...
    $report = @"
# FAST3 R26A / R26 Agent Launcher Report
...
"@
    ...
    Write-Section "FAST3 R26A / R26 agent launcher summary"
