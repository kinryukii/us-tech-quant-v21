...
$expectedControlRefs = @(
    $LimitsPath,
    $PromptPath,
    (Join-Path $RepoRoot "scripts\fast3\agent\run_fast3_r4_two_stage_direction_hard_r24_agent.ps1"),
...
$LockPath = Join-Path $StateRoot "active_hard_r24.lock.json"
...
"@ | Set-Content -LiteralPath $gitBlocker -Encoding ASCII
    ...
    $instruction = @"
Implement and unit-test FAST3 R4 hard-storage R2.4.
...
'@
    ...
    $testFile = Join-Path $RepoRoot "fast3\tests\unit\test_fast3_r4_two_stage_direction_hard_r24.py"
