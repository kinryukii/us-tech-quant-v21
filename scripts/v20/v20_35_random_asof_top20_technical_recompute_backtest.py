...
IN_TICKER_CACHE = ROOT / "inputs" / "v20" / "outcome_benchmark" / "yahoo_cache" / "v20_26" / "V20_26_YAHOO_TICKER_PRICE_CACHE.csv"
IN_BENCHMARK_CACHE = ROOT / "inputs" / "v20" / "outcome_benchmark" / "yahoo_cache" / "v20_26" / "V20_26_YAHOO_BENCHMARK_PRICE_CACHE.csv"
IN_UNIVERSE = CONSOLIDATION / "V20_26_REQUIRED_SYMBOL_UNIVERSE.csv"

OUT_DATE_POOL = CONSOLIDATION / "V20_35_RANDOM_SIGNAL_DATE_CANDIDATE_POOL.csv"
OUT_DATE_SAMPLE = CONSOLIDATION / "V20_35_SELECTED_RANDOM_SIGNAL_DATE_SAMPLE.csv"
OUT_TICKER_COVERAGE = CONSOLIDATION / "V20_35_HISTORICAL_PRICE_COVERAGE_AUDIT.csv"
OUT_BENCH_COVERAGE = CONSOLIDATION / "V20_35_BENCHMARK_PRICE_COVERAGE_AUDIT.csv"
OUT_FACTOR_MATRIX = CONSOLIDATION / "V20_35_ASOF_TECHNICAL_FACTOR_RECOMPUTE_MATRIX.csv"
OUT_FACTOR_AVAIL = CONSOLIDATION / "V20_35_ASOF_TECHNICAL_FACTOR_AVAILABILITY_SUMMARY.csv"
OUT_RANKING = CONSOLIDATION / "V20_35_ASOF_TECHNICAL_SCORE_AND_RANKING.csv"
OUT_TOP20 = CONSOLIDATION / "V20_35_ASOF_TOP20_SELECTIONS.csv"
OUT_TOP50 = CONSOLIDATION / "V20_35_ASOF_TOP50_SELECTIONS.csv"
OUT_TOP100 = CONSOLIDATION / "V20_35_ASOF_TOP100_SELECTIONS.csv"
OUT_ATTACH = CONSOLIDATION / "V20_35_FORWARD_OUTCOME_ATTACHMENT.csv"
OUT_RETURNS = CONSOLIDATION / "V20_35_EXPLORATORY_ROW_LEVEL_RETURNS.csv"
OUT_SIGNAL_SUM = CONSOLIDATION / "V20_35_EXPLORATORY_SIGNAL_DATE_SUMMARY.csv"
OUT_WINDOW_SUM = CONSOLIDATION / "V20_35_EXPLORATORY_FORWARD_WINDOW_SUMMARY.csv"
OUT_BUCKET_SUM = CONSOLIDATION / "V20_35_EXPLORATORY_TOP_BUCKET_SUMMARY.csv"
OUT_BENCH_REL_SUM = CONSOLIDATION / "V20_35_EXPLORATORY_BENCHMARK_RELATIVE_SUMMARY.csv"
OUT_PIT = CONSOLIDATION / "V20_35_STALE_LEAKAGE_PIT_GATE.csv"
OUT_BLOCKED_ENFORCE = CONSOLIDATION / "V20_35_BLOCKED_NON_PIT_FACTOR_ENFORCEMENT.csv"
OUT_DECISION = CONSOLIDATION / "V20_35_RANDOM_ASOF_BACKTEST_DECISION.csv"
OUT_NEXT = CONSOLIDATION / "V20_35_NEXT_STEP_DECISION_SUMMARY.csv"
REPORT = READ_CENTER / "V20_35_RANDOM_ASOF_TOP20_TECHNICAL_RECOMPUTE_BACKTEST_REPORT.md"
CURRENT_REPORT = READ_CENTER / "V20_CURRENT_RANDOM_ASOF_TOP20_TECHNICAL_RECOMPUTE_BACKTEST.md"
READ_FIRST = OPS / "V20_35_READ_FIRST.txt"

STAGE_NAME = "V20.35_RANDOM_ASOF_TOP20_TECHNICAL_RECOMPUTE_BACKTEST"
PASS_STATUS = "PASS_V20_35_RANDOM_ASOF_TOP20_TECHNICAL_RECOMPUTE_BACKTEST"
BLOCKED_STATUS = "BLOCKED_V20_35_RANDOM_ASOF_TOP20_TECHNICAL_RECOMPUTE_BACKTEST"
RANDOM_SEED = 2035
...
def main() -> int:
    ...
    for selected in selected_all:
        ...
        for window in FORWARD_WINDOWS:
            ...
            if not pit_ok or not bench_align:
                leakage_blockers += 1
            if not formula_ok and available:
            ...
            ret = {
                **attach,
                ...
                "row_included": tf(available and pit_ok and bench_align and formula_ok),
    ...
    decision_out = [{
        "decision_id": "V20_35_RANDOM_ASOF_BACKTEST_DECISION",
    ...
    next_rows = [{
        ...
        "STATUS": status,
        "V20_34_GATE_READY": tf(v20_34_ready),
    ...
    report = f"""# V20.35 Random Asof Top20 Technical Recompute Backtest
...
V20.34 gate ready: {tf(v20_34_ready)}
...
V20.35 did not create official recommendations, trading signals, broker/order/execution code, official ranking mutations, official factor weight mutations, dynamic weighting, portfolio-level backtests, equity curves, Sharpe/Sortino/final performance claims, V21 outputs, or V19.21 outputs.
"""
...
"""
    ...
    required_outputs = [
        OUT_DATE_POOL, OUT_DATE_SAMPLE, OUT_TICKER_COVERAGE, OUT_BENCH_COVERAGE,
        ...
        OUT_BUCKET_SUM, OUT_BENCH_REL_SUM, OUT_PIT, OUT_BLOCKED_ENFORCE,
        OUT_DECISION, OUT_NEXT, REPORT, CURRENT_REPORT, READ_FIRST,
    ]
    missing = [path for path in required_outputs if not path.exists()]
    if missing:
        raise RuntimeError("Missing V20.35 outputs: " + ", ".join(rel(path) for path in missing))

    print(f"STATUS={status}")
    print("FILES_CHANGED=scripts/v20/v20_35_random_asof_top20_technical_recompute_backtest.py;scripts/v20/run_v20_35_random_asof_top20_technical_recompute_backtest.ps1")
    print("OUTPUTS_CREATED=" + ";".join(rel(path) for path in required_outputs))
    print(f"V20_34_GATE_READY={tf(v20_34_ready)}")
