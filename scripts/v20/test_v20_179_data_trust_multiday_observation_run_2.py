#!/usr/bin/env python
"""Tests for V20.179 DATA_TRUST multiday observation run 2."""
...
SCRIPT = ROOT / "scripts" / "v20" / "v20_179_data_trust_multiday_observation_run_2.py"
...
OUTPUTS = [
    ...
    READ_CENTER / "V20_179_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_2_REPORT.md",
]
PROTECTED = [
...
def test_data_trust_multiday_observation_run_2() -> None:
    ...
    for expected in [
        ...
        "RUN_2_DATA_TRUST_DISCLOSURE_CANDIDATE_COUNT=40",
        ...
        "RUN_2_DISCLOSURE_PASS=TRUE",
        "RUN_2_NO_MUTATION_GUARD_PASS=TRUE",
        "RUN_2_ZERO_WEIGHT_GUARD_PASS=TRUE",
        "RUN_2_OFFICIAL_USE_GUARD_PASS=TRUE",
        "RUN_2_VS_RUN_1_COMPARISON_PASS=TRUE",
        "READY_FOR_V20_180_MULTIDAY_OBSERVATION_RUN_3=TRUE",
    ...
    assert all(row["run_2_disclosure_pass"] == "TRUE" for row in disclosure)
    ...
    assert all(row["run_2_vs_run_1_comparison_pass"] == "TRUE" for row in comparison)
    assert all(row["data_trust_caused_ranking_mutation"] == "FALSE" for row in comparison)
    assert gate["final_status"] == PASS_STATUS
    assert gate["ready_for_v20_180_multiday_observation_run_3"] == "TRUE"
    ...
    assert "Run 2 confirms" in report


if __name__ == "__main__":
    test_data_trust_multiday_observation_run_2()
    print("PASS test_v20_179_data_trust_multiday_observation_run_2")
