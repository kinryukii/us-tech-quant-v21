...
$StageScript = Join-Path $ScriptDir "v20_9_r1_relative_path_and_staging_contract_repair.py"

Write-Host "STAGE_NAME=V20.9-R1_RELATIVE_PATH_AND_STAGING_CONTRACT_REPAIR"
...
try {
    python $StageScript
    if ($LASTEXITCODE -ne 0) { throw "V20.9-R1 failed with exit code $LASTEXITCODE" }
}
