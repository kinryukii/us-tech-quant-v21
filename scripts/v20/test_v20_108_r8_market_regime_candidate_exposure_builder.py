...
SCRIPT = ROOT / "scripts" / "v20" / "v20_108_r8_market_regime_candidate_exposure_builder.py"
TEST_SCRIPT = ROOT / "scripts" / "v20" / "test_v20_108_r8_market_regime_candidate_exposure_builder.py"
WRAPPER = ROOT / "scripts" / "v20" / "run_v20_108_r8_market_regime_candidate_exposure_builder.ps1"
...
ACCEPTED = {
    "PASS_V20_108_R8_MARKET_REGIME_CANDIDATE_EXPOSURE_BUILDER",
    "PARTIAL_PASS_V20_108_R8_MARKET_REGIME_CANDIDATE_EXPOSURE_BUILDER_WITH_PARTIAL_COVERAGE",
    "BLOCKED_V20_108_R8_NO_SAFE_MARKET_REGIME_CANDIDATE_EXPOSURE_FOUND",
}
...
def test_wrapper_runs_and_outputs_created() -> None:
    ...
    for path in [OUT_SOURCE, OUT_AUDIT, OUT_COMPONENT, OUT_VALIDATION, OUT_COVERAGE, OUT_GATE, REPORT]:
        assert_true(path.exists() and path.stat().st_size > 0, f"missing output: {path}")


def test_source_scope_and_audit() -> None:
    rows = read_csv(OUT_SOURCE)
    audit = read_csv(OUT_AUDIT)
    assert_true(len(rows) == 315, "all 315 candidates not represented")
    assert_true(audit, "source audit empty")
    for row in rows + audit:
        assert_safety(row)
    assert_true(all(row["source_rank_or_score_used_as_market_regime"] == "FALSE" for row in audit), "source score used")
    assert_true(all(row["baseline_rank_used_as_market_regime"] == "FALSE" for row in audit), "baseline rank used")
    materialized = [row for row in rows if row["market_regime_contribution"]]
    for row in materialized:
        assert_true(row["market_regime_raw_columns_used"] or row["market_regime_categorical_mappings_used"], f"materialized without audited exposure: {row}")
        assert_true(row["global_regime_copied_without_exposure_conditioning"] == "FALSE", f"global copied: {row}")
    if not materialized:
        assert_true(all(row["market_regime_source_status"] == "MISSING_CANDIDATE_LEVEL_MARKET_REGIME_SOURCE" for row in rows), "missing source not explicit")
...
def test_validation_gates_and_report() -> None:
    ...
    for row in component + validation + coverage + gate:
        assert_safety(row)
    assert_true(all(row["fabricated_values_created"] == "FALSE" for row in component), "fabricated values")
    assert_true(all(row["proxy_values_activated"] == "FALSE" for row in component), "proxy values")
    assert_true(all(row["global_regime_copied_without_exposure_conditioning"] == "FALSE" for row in component), "global copied")
    v = validation[0]
    for field in [
        "fabricated_values_created", "proxy_values_activated", "source_rank_or_score_used",
        "baseline_rank_used", "global_regime_copied_without_exposure_conditioning",
        ...
        assert_true(v[field] == "FALSE", f"{field} should be false")
...
def main() -> int:
    ...
    test_source_scope_and_audit()
    test_validation_gates_and_report()
    print("PASS_V20_108_R8_MARKET_REGIME_CANDIDATE_EXPOSURE_BUILDER_TESTS")
    return 0
