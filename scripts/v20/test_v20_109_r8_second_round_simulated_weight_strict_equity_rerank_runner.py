#!/usr/bin/env python
"""Tests for V20.109-R8 second-round simulated strict equity rerank runner."""
...
SCRIPT = ROOT / "scripts" / "v20" / "v20_109_r8_second_round_simulated_weight_strict_equity_rerank_runner.py"
R7_SCENARIOS = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R7_ADDITIONAL_SIMULATION_SCENARIOS.csv"
...
OUT_RERANK = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R8_SECOND_ROUND_SIMULATED_WEIGHT_STRICT_EQUITY_RERANK.csv"
OUT_DELTA = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R8_SECOND_ROUND_SIMULATED_RANK_DELTA_AUDIT.csv"
OUT_COMPONENT = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R8_SECOND_ROUND_SIMULATED_SCORE_COMPONENT_AUDIT.csv"
OUT_VALIDATION = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R8_SECOND_ROUND_SIMULATED_RERANK_VALIDATION.csv"
OUT_GATE = ROOT / "outputs" / "v20" / "consolidation" / "V20_109_R8_NEXT_STAGE_GATE.csv"
OUT_REPORT = ROOT / "outputs" / "v20" / "read_center" / "V20_109_R8_SECOND_ROUND_SIMULATED_WEIGHT_STRICT_EQUITY_RERANK_REPORT.md"

PAIRS = [
    ("fundamental_contribution", "second_round_fundamental_weight"),
    ("technical_contribution", "second_round_technical_weight"),
    ("strategy_contribution", "second_round_strategy_weight"),
    ("risk_contribution", "second_round_risk_weight"),
    ("market_regime_contribution", "second_round_market_regime_weight"),
    ("data_trust_contribution", "second_round_data_trust_weight"),
]
...
def test_second_round_simulated_rerank_runner() -> None:
    result = subprocess.run(["python", str(SCRIPT)], cwd=ROOT, text=True, capture_output=True, check=True)
    stdout = result.stdout
    assert any(status in stdout for status in [
        "PASS_V20_109_R8_SECOND_ROUND_SIMULATED_WEIGHT_STRICT_EQUITY_RERANK_RUNNER",
        "PARTIAL_PASS_V20_109_R8_SECOND_ROUND_SIMULATED_RERANK_WITH_LIMITED_EVIDENCE",
        "BLOCKED_V20_109_R8_MISSING_SECOND_ROUND_SIMULATION_SCENARIOS_OR_INPUT_MATRIX",
    ])
    for expected in [
        "SCENARIO_COUNT=8",
        ...
        "V20_109_R9_SECOND_ROUND_EFFECTIVENESS_COMPARISON_ALLOWED=TRUE",
    ...
    scenarios = [row for row in read_csv(R7_SCENARIOS) if row["weight_sum_valid"] == "TRUE"]
    ...
    assert all(row["targets_prior_failure_area"] == "TRUE" for row in rerank)
    assert all(row["targeted_forward_window"] == "120D" for row in rerank)
    assert all(row["targeted_topn_group"] == "20" for row in rerank)

    for scenario in scenarios:
        sid = scenario["simulation_scenario_id"]
        rows = [row for row in rerank if row["simulation_scenario_id"] == sid]
        assert len(rows) == 297
        ranks = sorted(int(row["second_round_simulated_shadow_rank"]) for row in rows)
        assert ranks == list(range(1, 298))
        for row in rows[:10]:
            total = sum(Decimal(row[c]) * Decimal(row[w]) for c, w in PAIRS)
            assert row["second_round_simulated_shadow_weighted_score"] == q(total)
            assert row["second_round_weight_sum"] == "1.0000000000"
    ...
    assert v["second_round_simulated_rerank_created"] == "TRUE"
    ...
    assert gate[0]["second_round_simulated_rerank_created"] == "TRUE"
    assert gate[0]["prior_failure_area_targeted"] == "TRUE"
    assert gate[0]["v20_109_r9_second_round_effectiveness_comparison_allowed"] == "TRUE"
...
if __name__ == "__main__":
    test_second_round_simulated_rerank_runner()
    print("PASS_V20_109_R8_SECOND_ROUND_SIMULATED_RERANK_RUNNER_TESTS")
