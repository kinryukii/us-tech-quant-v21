...
$StageScript = Join-Path $ScriptDir "v20_87_downside_risk_evidence_export.py"

Write-Host "STAGE_NAME=V20.87_DOWNSIDE_RISK_EVIDENCE_EXPORT"
...
try {
    ...
    if (-not $DetectedStatus) {
        throw "V20.87 did not report an allowed final status"
    }
    if ($ExitCode -ne 0 -and $DetectedStatus -like "PASS_*") {
        throw "V20.87 reported pass status with non-zero exit code $ExitCode"
    }
    ...
    Write-Host "BLOCKED_V20_87_MISSING_REQUIRED_UPSTREAM_EVIDENCE"
