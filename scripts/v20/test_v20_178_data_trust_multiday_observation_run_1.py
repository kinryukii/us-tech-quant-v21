#!/usr/bin/env python
"""Tests for V20.178 DATA_TRUST multiday observation run 1."""
...
SCRIPT = ROOT / "scripts" / "v20" / "v20_178_data_trust_multiday_observation_run_1.py"
...
OUTPUTS = [
    ...
    READ_CENTER / "V20_178_DATA_TRUST_MULTIDAY_OBSERVATION_RUN_1_REPORT.md",
]
PROTECTED = [
...
def test_data_trust_multiday_observation_run_1() -> None:
    ...
    for expected in [
        ...
        "RUN_1_DATA_TRUST_DISCLOSURE_CANDIDATE_COUNT=40",
        ...
        "RUN_1_DISCLOSURE_PASS=TRUE",
        "RUN_1_NO_MUTATION_GUARD_PASS=TRUE",
        "RUN_1_ZERO_WEIGHT_GUARD_PASS=TRUE",
        "RUN_1_OFFICIAL_USE_GUARD_PASS=TRUE",
        "READY_FOR_V20_179_MULTIDAY_OBSERVATION_RUN_2=TRUE",
    ...
    assert all(row["run_1_disclosure_pass"] == "TRUE" for row in disclosure)
    ...
    assert gate["ready_for_v20_179_multiday_observation_run_2"] == "TRUE"
    assert gate["ready_for_official_use"] == "FALSE"
    assert_safety([gate, *run, *disclosure[:3]])
    report = OUTPUTS[-1].read_text(encoding="utf-8")
    assert "Run 1 confirms" in report


if __name__ == "__main__":
    test_data_trust_multiday_observation_run_1()
    print("PASS test_v20_178_data_trust_multiday_observation_run_1")
