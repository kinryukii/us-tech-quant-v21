...
$StageScript = Join-Path $ScriptDir "v20_43_daily_operator_report_dry_run.py"

Write-Host "STAGE_NAME=V20.43_DAILY_OPERATOR_REPORT_DRY_RUN"
Write-Host "DRY_RUN_ONLY=TRUE"
Write-Host "RESEARCH_ONLY=TRUE"
Write-Host "USE_EXISTING_OUTPUTS_ONLY=TRUE"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V20.43 stage failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
